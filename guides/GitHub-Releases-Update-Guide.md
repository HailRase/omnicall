# GitHub Releases и проверка обновлений (OmniCall)
...
| --- | --- | --- |
| **Manifest** (JSON с версией) | `.env.production` → `VITE_UPDATE_MANIFEST_URL` | `https://raw.githubusercontent.com/HailRase/omnicall-releases/main/update-manifest.json` |
| **Установщики** | Внутри manifest: `downloadUrl`, `platforms` | `https://github.com/HailRase/omnicall-releases/releases/latest` |
...
Шаблон: `OmniCall-${version}-${platform}-${arch}.${ext}` (`productName: OmniCall` в `electron-builder.yml`).

| Платформа | Пример для `0.0.1` |
| --- | --- |
| Windows NSIS | `OmniCall-0.0.1-win-x64.exe` |
| macOS DMG (arm64) | `OmniCall-0.0.1-mac-arm64.dmg` |
| macOS DMG (x64) | `OmniCall-0.0.1-mac-x64.dmg` |
| Linux AppImage | `OmniCall-0.0.1-linux-x86_64.AppImage` (CI) или `…-linux-x64…` локально — сверьте `dist/linux/` |
| Linux deb | `OmniCall-0.0.1-linux-amd64.deb` |
...
1. Убедитесь, что `update-manifest.json` есть в **`main`** репозитория [`omnicall-releases`](https://github.com/HailRase/omnicall-releases).
2. В `.env.production` задан `VITE_UPDATE_MANIFEST_URL` (raw URL `omnicall-releases`).
...
3. Workflow **Release** в `softphone-electron` собирает win/mac/linux и публикует assets в [`omnicall-releases`](https://github.com/HailRase/omnicall-releases) (`softprops/action-gh-release`)

Ручная загрузка бинарников **не нужна**, если CI зелёный.

### Вручную (legacy / hotfix)

1. Откройте https://github.com/HailRase/omnicall-releases/releases
2. **Draft a new release**
3. **Tag:** `v0.0.1` (префикс `v` + semver из `package.json`)
4. **Title:** `OmniCall 0.0.1`
...
- Прямая ссылка на файл (`platforms.*`):
  `https://github.com/HailRase/omnicall-releases/releases/download/v0.0.2/OmniCall-0.0.2-win-x64.exe`
...
5. `git tag vX.Y.Z` → `git push origin vX.Y.Z` → CI публикует Release assets в **omnicall-releases**
...
Push тега `vX.Y.Z` после manifest в `main` → сборка в **softphone-electron** → publish в **omnicall-releases**.
...
Для дополнения уже существующего Release без нового тега: собрать локально или push тега; при необходимости **Edit release → Attach binaries** на omnicall-releases.
...
```bash
curl -s "https://raw.githubusercontent.com/HailRase/omnicall-releases/main/update-manifest.json"
```
...
Исходный код — **private** `HailRase/softphone-electron`. Публичная дистрибуция — **`HailRase/omnicall-releases`** (только README, manifest, Releases).

Чеклист переноса: `guides/Distribution-Migration-Checklist.md`.  
Секрет CI: `OMNICALL_RELEASES_TOKEN` (write на `omnicall-releases`).
