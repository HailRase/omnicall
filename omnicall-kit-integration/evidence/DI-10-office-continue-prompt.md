...
# Repo: softphone-electron · branch: `feature/omnicall-kit`
...
2. `omnicall-kit-integration/WORK-UNITS.md` — DI-10 status (`review` / awaiting `/sdk-review`)
3. `omnicall-kit-integration/evidence/DI-10-compatibility-e2e-p12-close.md` — canonical DI-10 evidence
4. `omnicall-kit-integration/SMOKE-CHECKLIST.md` — Record + OPEN cells
...
8. `omnicall-kit/evidence/SDK-10-release-candidate.md` — Mode A done; Mode B / `latest` blocked
9. `omnicall-kit/docs/guide/compatibility-matrix.md`
...
| Item | Expected state |
| --- | --- |
| Branch | `feature/omnicall-kit` (pull latest) |
| Desktop version | `0.11.2` |
| SDK API | `@softomnitel/omnicall-kit` **47** / `@softomnitel/omnicall-protocol` **169** |
| DI-00…DI-09 | `done` |
...
- win-unpacked / NSIS / MSI `OmniCall-0.11.2`
- Node packaged smoke 5/5 + Edge browser smoke 2/2
- Reports under `omnicall-kit-integration/evidence/DI-10-*-smoke-report.json`
...
1. **On office PC:** `git pull` on `feature/omnicall-kit`; `npm ci` (desktop root + `omnicall-kit/` if needed); confirm clean `git status`.
...
5. **After DI-10 `done`:** human chooses path for remaining OPEN cells:
   ...
   - **Path C — defer** product close; keep F-011 `in progress`; do not publish `@softomnitel/omnicall-*` `latest`.
...
4. Планируем ли в ближайшие дни **npm publish** `@softomnitel/omnicall-*` (хотя бы `rc`), или это явно later?
