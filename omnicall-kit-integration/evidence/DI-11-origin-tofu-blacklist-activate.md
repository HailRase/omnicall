...
- `src/main/sdk/registerSdkGateway.ts` (always-on except `OMNICALL_SDK_GATEWAY=0`)
...
- `settingsSections.ts` — OmniCall Kit nav leaf (`labelKey`/`iconId` distinct from OCP)
...
### SDK
```text
cd omnicall-kit && npx vitest run \
...
- Adapter seeds only `OMNICALL_SDK_ALLOWED_ORIGINS` / constructor allowlist as `allowed`.
...
| Blocker: env seed re-allows blacklist | `mergePersistedOriginTrustWithEnvSeed` — persisted `denied` always wins over `OMNICALL_SDK_ALLOWED_ORIGINS` |
...
npm run typecheck → PASS
npm run i18n:check → PASS
(cd omnicall-kit && npx vitest run origin-policy-*.test.ts) → 2 files / 6 passed
```
