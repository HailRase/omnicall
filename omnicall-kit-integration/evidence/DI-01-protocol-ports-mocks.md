...
| SDK-01 `done` | yes |
| SDK-02 `@softomnitel/omnicall-protocol` + fixtures `done` | yes |
...
- Root dependency: `"@softomnitel/omnicall-protocol": "file:omnicall-kit/packages/protocol"`
- Fixtures loaded from `omnicall-kit/packages/protocol/fixtures/**` (same tree as SDK-02)
...
Root `eslint .` previously also walked nested `omnicall-kit/**` (separate workspace with its own ESLint; includes in-progress SDK-03 WIP). DI-01 added `omnicall-kit/**` to desktop root eslint `ignores` so desktop and SDK lint stay independent. SDK continues to lint via `cd omnicall-kit && npm run lint`.

## Boundary proof

- `src/ports/integration/sdk-dependency-boundary.test.ts` — Domain free of `@softomnitel/omnicall-protocol` / zod / electron / ws; renderer components/stores free of gateway ports
