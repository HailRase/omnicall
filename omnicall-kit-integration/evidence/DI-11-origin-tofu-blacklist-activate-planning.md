...
**Work unit:** `omnicall-kit-integration/WORK-UNITS.md` — DI-11 `pending`  
...
1. SDK loopback gateway always listens at process start (primary instance); Settings
   enable/disable listener toggle removed (env `OMNICALL_SDK_GATEWAY=0` kill-switch only).
...
7. Settings → Integrations → OmniCall Kit available pre-auth; OCP Module stays gated.
