...
| SDK-01 + SDK-02 `@softomnitel/omnicall-protocol` + fixtures `done` | yes |
...
- Loopback HTTP discovery: `GET http://127.0.0.1:<port>/omnicall/v1/discovery` (ADR-0015)
- WS upgrade: `/omnicall/v1/ws` via `ws` `WebSocketServer({ noServer: true })`
...
| JSON depth / keys / arrays | `@softomnitel/omnicall-protocol` validators |
| Max connections | 8 |
...
- Disable via `OMNICALL_SDK_GATEWAY=0` (settings UX still DI-09)
...
- Default gateway enablement until DI-09 settings (disable with `OMNICALL_SDK_GATEWAY=0`)
