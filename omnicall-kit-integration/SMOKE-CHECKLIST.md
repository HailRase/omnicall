...
- [ ] `window:show` brings OmniCall above other apps when visible but occluded (not only taskbar flash).
- [ ] Incoming call raises/restores OmniCall above other apps.
- [ ] Outgoing Connecting raises/restores OmniCall above other apps.
- [ ] SDK Origin trust (TOFU) pending raises OmniCall above other apps.
- [ ] SDK Origin TOFU / pairing pending raises OmniCall and shows root `SdkConnectCeremonyModal` (no Settings auto-open).
