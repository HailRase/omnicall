# MASTER PROMPT — Axatalk SDK Demo Stand (`sdk-demo/`)
...
```markdown
# AGENT TASK: Build disposable `sdk-demo/` — expert Axatalk SDK live demo stand

## Role

You are a senior front-end + integration engineer. Build a **beautiful, self-contained, deletable**
demo stand that exercises the real Axatalk Desktop local gateway via `@axata/axatalk-sdk`.
...
2. **Disposable:** deleting `sdk-demo/` must leave the product clean.
   ...
   - Do **not** change desktop `src/`, `axatalk-sdk/packages/**` production API, or SemVer.
...
4. **Load the local SDK** (packages are not on public npm yet):
   - Packages: `@axata/axatalk-sdk` + `@axata/axatalk-protocol` under `axatalk-sdk/`.
   - Build parent first if needed: `cd axatalk-sdk && npm run build`.
...
7. **Default WS URL:** `ws://127.0.0.1:17341/axatalk/v1/ws` (editable in UI). Optional
   discovery helper if documented in desktop/SDK guides — do not invent insecure endpoints.

## Read first (repo)

1. `axatalk-sdk/docs/guide/pairing-quick-start.md`
2. `axatalk-sdk/docs/guide/api-reference.md`
3. `axatalk-sdk/docs/guide/capabilities.md`
4. `axatalk-sdk/docs/guide/events.md`
...
- Strong product identity: **Axatalk SDK Demo** as hero-level title.
...
### A. Connect & pair
- Connect / Disconnect
- Show pairing-required banner: “Approve this Origin in Axatalk → Settings → Integrations → SDK”
...
# 2) Start Axatalk Desktop (dev or packaged) with gateway enabled and Origin allowlisted
# Example env for dev:
#   AXATALK_SDK_GATEWAY=1
...
Use Agent mode on branch `feature/axatalk-sdk` with workspace root = softphone repo.
)
