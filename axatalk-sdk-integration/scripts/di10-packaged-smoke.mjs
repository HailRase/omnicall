 ...
 *
 * Usage (desktop already running with AXATALK_SDK_ALLOWED_ORIGINS set):
...
const DISCOVERY_PATH = "/axatalk/v1/discovery";
const WS_PATH = "/axatalk/v1/ws";
const APPROVED_ORIGIN =
  process.env["AXATALK_SDK_SMOKE_ORIGIN"] ?? "https://di10-test.example";
...
async function main() {
  ...
  try {
    ...
    if (discovery.status === 200 && discovery.text.includes("axatalk")) {
      ...
      record(
        "packaged.discovery",
        "FAIL",
        `HTTP ${discovery.status}; is Axatalk running with gateway enabled?`,
      );
    }
  } catch (error) {
    record(
...

  {
    ...
    if (outcome !== "open") {
      record(
        ...
        "FAIL",
        `expected open, got ${outcome}; set AXATALK_SDK_ALLOWED_ORIGINS=${APPROVED_ORIGIN}`,
      );
      ws.terminate();
    } else {
