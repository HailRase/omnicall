#!/usr/bin/env node
import { spawnSync } from 'node:child_process';
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const packages = ['protocol', 'sdk'];

/** SDK-08 allowlist — auth + read + calls + operator/logout + activate result. */
const SDK_ALLOWED_SYMBOLS = new Set([
  'ActivateProfileMode',
  'ActivateProfileResult',
  'ApplicationIdentity',
  'AuthClient',
  'AuthClientOptions',
  'AuthSessionSnapshot',
  'OmniCallAccountApi',
  'OmniCallCallsApi',
  'OmniCallClient',
  'OmniCallClientError',
  'OmniCallClientOptions',
  'OmniCallEvent',
  'OmniCallEventOf',
  'OmniCallDiscoveryOptions',
  'OmniCallOperatorApi',
  'OmniCallWindowApi',
  'CallMutationResult',
  'CapabilityId',
  'CONNECTION_STATES',
  'ConflictErrorDetails',
  'ConnectionState',
  'DiagnosticEvent',
  'DiagnosticLevel',
  'DiagnosticResult',
  'DiagnosticsSink',
  'DiscoveryDocument',
  'FakeScheduler',
  'HeartbeatPolicy',
  'InteractionRequiredDetails',
  'JitterSource',
  'LogoutResult',
  'OperationFailedDetails',
  'OperatorFinishAppealResult',
  'OperatorReason',
  'OperatorReasonsResult',
  'OperatorStatusChangeKind',
  'OperatorStatusChangeResult',
  'PUBLIC_EVENT_TYPES',
  'PairingRequiredInfo',
  'PairingProfile',
  'PopKeyStore',
  'ProtocolErrorCode',
  'ProtocolVersion',
  'PublicEventType',
  'PublicOperatorStatus',
  'ReconnectPolicy',
  'Revision',
  'Scheduler',
  'SDK_ACTIVATE_CLIENT_TIMEOUT_MS',
  'SDK_ACTIVATE_CONSENT_TTL_MS',
  'SDK_ACTIVATE_OCP_AUTH_BUDGET_MS',
  'SDK_ACTIVATE_SIP_ONLY_AUTH_BUDGET_MS',
  'SDK_VERSION',
  'SnapshotCallSummary',
  'SnapshotMessage',
  'SnapshotSections',
  'StoredPopIdentity',
  'TimerHandle',
  'TransportCloseInfo',
  'TransportErrorInfo',
  'TransportFactory',
  'TransportPort',
  'WireJsonObject',
  'WireJsonValue',
  'WaitUntilTimeoutError',
  'createAuthClient',
  'discoverOmniCallDesktop',
  'createOmniCallClient',
  'createBrowserJitterSource',
  'createBrowserScheduler',
  'createBrowserWebSocketTransport',
  'createFakeScheduler',
  'createFixedJitterSource',
  'createIndexedDbPopKeyStore',
  'createMemoryPopKeyStore',
  'createRecordingDiagnosticsSink',
  'isOmniCallClientError',
  'isConflictError',
  'isInteractionRequiredError',
  'isOperationFailedError',
  'isOriginBlockedError',
  'readConflictErrorDetails',
  'readInteractionRequiredDetails',
  'readOperationFailedDetails',
  'BrowserWebSocketConstructor',
  'BrowserWebSocketLike',
  'CreateBrowserWebSocketTransportOptions'
]);

/** Top-level exports forbidden (namespaced methods are not separate symbols). */
const SDK_FORBIDDEN_SYMBOLS = new Set([
  'activateProfile',
  'logout',
  'changeStatus',
  'finishAppeal',
  'getReasons'
]);

/**
 * @param {string} command
 * @param {string[]} args
 * @param {string} cwd
 */
function run(command, args, cwd) {
  console.log(`\n> ${command} ${args.join(' ')} (cwd=${path.relative(root, cwd) || '.'})`);
  const result = spawnSync(command, args, {
    cwd,
    stdio: 'inherit',
    shell: process.platform === 'win32',
    env: process.env
  });
  if (result.status !== 0) {
    process.exit(result.status ?? 1);
  }
}

/**
 * @param {string} report
 * @returns {string[]}
 */
function listPublicSymbols(report) {
  const typed = [
    ...report.matchAll(
      /^\s*export\s+(?:declare\s+)?(?:class|function|const|interface|type|enum)\s+(\w+)/gm
    )
  ].map((m) => m[1]);
  // API Extractor re-exports: `export { SnapshotMessage }`
  const braced = [...report.matchAll(/^export \{ (\w+) \}$/gm)].map((m) => m[1]);
  return [...new Set([...typed, ...braced].filter((name) => typeof name === 'string'))];
}

const reportDir = path.join(root, 'etc', 'api');
const tempDir = path.join(root, 'temp', 'api');
fs.mkdirSync(reportDir, { recursive: true });
fs.mkdirSync(tempDir, { recursive: true });

for (const name of packages) {
  const pkgDir = path.join(root, name === 'protocol' ? 'packages/protocol' : 'packages/sdk');
  const distEntry = path.join(pkgDir, 'dist', 'index.d.ts');
  if (!fs.existsSync(distEntry)) {
    console.error(`Missing build output for @softomnitel/omnicall-${name}: ${distEntry}`);
    console.error('Run `npm run build` before api:check.');
    process.exit(1);
  }
  run(
    'npx',
    [
      'api-extractor',
      'run',
      '--local',
      '--verbose',
      '--typescript-compiler-folder',
      path.join(root, 'node_modules', 'typescript')
    ],
    pkgDir
  );
}

for (const name of packages) {
  const reportPath = path.join(reportDir, `${name}.api.md`);
  if (!fs.existsSync(reportPath)) {
    console.error(`API report missing: ${reportPath}`);
    process.exit(1);
  }
  const report = fs.readFileSync(reportPath, 'utf8');
  const symbols = listPublicSymbols(report);

  if (name === 'sdk') {
    if (symbols.length === 0) {
      console.error(`Expected SDK-08 public surface in ${reportPath}`);
      process.exit(1);
    }
    for (const symbol of symbols) {
      if (SDK_FORBIDDEN_SYMBOLS.has(symbol)) {
        console.error(`Forbidden public symbol in SDK API: ${symbol}`);
        process.exit(1);
      }
      if (!SDK_ALLOWED_SYMBOLS.has(symbol)) {
        console.error(`Unexpected public SDK symbol (not in SDK-08 allowlist): ${symbol}`);
        process.exit(1);
      }
    }
    if (!symbols.includes('createOmniCallClient')) {
      console.error('SDK API must export createOmniCallClient');
      process.exit(1);
    }
    if (!symbols.includes('OmniCallClient')) {
      console.error('SDK API must export OmniCallClient');
      process.exit(1);
    }
    if (!symbols.includes('OmniCallCallsApi')) {
      console.error('SDK API must export OmniCallCallsApi');
      process.exit(1);
    }
    if (!symbols.includes('OmniCallOperatorApi')) {
      console.error('SDK API must export OmniCallOperatorApi');
      process.exit(1);
    }
    if (!symbols.includes('OmniCallAccountApi')) {
      console.error('SDK API must export OmniCallAccountApi');
      process.exit(1);
    }
    const clientMatch = report.match(
      /export type OmniCallClient = \{([\s\S]*?)\n\};/
    );
    if (clientMatch !== null) {
      const body = clientMatch[1];
      if (/^\s*readonly originate:/m.test(body)) {
        console.error(
          'Root-level originate is forbidden; use namespaced OmniCallCallsApi'
        );
        process.exit(1);
      }
      if (/^\s*readonly logout:/m.test(body)) {
        console.error(
          'Root-level logout is forbidden; use namespaced OmniCallAccountApi'
        );
        process.exit(1);
      }
      if (/^\s*readonly changeStatus:/m.test(body)) {
        console.error(
          'Root-level changeStatus is forbidden; use namespaced OmniCallOperatorApi'
        );
        process.exit(1);
      }
      if (/^\s*readonly finishAppeal:/m.test(body)) {
        console.error(
          'Root-level finishAppeal is forbidden; use namespaced OmniCallOperatorApi'
        );
        process.exit(1);
      }
      if (/^\s*readonly activateProfile:/m.test(body)) {
        console.error('Root-level activateProfile is forbidden (SDK-08)');
        process.exit(1);
      }
    }
    if (!report.includes('readonly calls:')) {
      console.error('OmniCallClient must expose namespaced calls API');
      process.exit(1);
    }
    if (!report.includes('readonly operator:')) {
      console.error('OmniCallClient must expose namespaced operator API');
      process.exit(1);
    }
    if (!report.includes('finishAppeal')) {
      console.error('OmniCallOperatorApi must expose finishAppeal');
      process.exit(1);
    }
    if (!report.includes('readonly account:')) {
      console.error('OmniCallClient must expose namespaced account API');
      process.exit(1);
    }
    if (!report.includes('activateProfile')) {
      console.error('OmniCallAccountApi must expose activateProfile (SDK-08)');
      process.exit(1);
    }
    if (!report.includes('readonly logout:')) {
      console.error('OmniCallAccountApi must expose logout (single-shot)');
      process.exit(1);
    }
    if (!symbols.includes('LogoutResult')) {
      console.error('SDK API must export LogoutResult');
      process.exit(1);
    }
    if (!symbols.includes('ActivateProfileResult')) {
      console.error('SDK API must export ActivateProfileResult');
      process.exit(1);
    }
    if (
      /\bpassword\b/i.test(report) ||
      /\bapiKey\b/i.test(report) ||
      /\bsipPassword\b/i.test(report)
    ) {
      console.error('SDK API report must not contain secret field names');
      process.exit(1);
    }
    console.log(
      `API report OK (sdk surface ${symbols.length} symbols): ${path.relative(root, reportPath)}`
    );
    continue;
  }

  if (symbols.includes('OmniCallClient')) {
    console.error(`Forbidden public symbol OmniCallClient in ${reportPath}`);
    process.exit(1);
  }
  if (symbols.length === 0) {
    console.error(`Expected public protocol API symbols in ${reportPath}`);
    process.exit(1);
  }
  console.log(
    `API report OK (protocol surface ${symbols.length} symbols, no OmniCallClient): ${path.relative(root, reportPath)}`
  );
}

console.log('\napi:check PASS');
