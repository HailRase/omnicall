...
- [x] SDK-02 — `@softomnitel/omnicall-protocol` (`done`)
...
> Initialize the OmniCall Kit npm workspace exactly as specified by SDK-00 in
...
- Workspace files: `omnicall-kit/package.json`, `packages/protocol`, `packages/sdk`, `docs/DEPENDENCIES.md`
- Verification output: `omnicall-kit/evidence/SDK-00-workspace.md`
- CI: `.github/workflows/omnicall-kit-ci.yml`
- API reports: `omnicall-kit/etc/api/protocol.api.md`, `omnicall-kit/etc/api/sdk.api.md`
...
- Browser spike: `omnicall-kit/evidence/SDK-01-browser-spike.md`
- Unit evidence: `omnicall-kit/evidence/SDK-01-protocol-adrs.md`
- Fixtures format: `omnicall-kit/docs/COMPATIBILITY-FIXTURES.md`
...
> Implement `@softomnitel/omnicall-protocol` from the approved protocol ADRs. Treat every boundary value
...
- [x] desktop consumes the same fixtures successfully. *(closed by DI-01 — `omnicall-kit-integration/evidence/DI-01-protocol-ports-mocks.md`; byte-identical corpus under `packages/protocol/fixtures/**`)*
- [x] package and type tests pass.

Evidence:

- Public API report: `omnicall-kit/etc/api/protocol.api.md`
- Fixture paths: `omnicall-kit/packages/protocol/fixtures/`
- Unit evidence: `omnicall-kit/evidence/SDK-02-protocol-package.md`
- Desktop DI-01 consume: documented in evidence (DI-01 still `pending`; not faked)
- Desktop DI-02 evidence: N/A
- Reviewer: `/sdk-review` **PASS** 2026-07-20 — independent rebuild/tests/lint/preflight PASS; zod@4.4.3 protocol-only; fixtures byte-stable; no OmniCallClient; DI-01 consume honesty OK. Post-review fix: CapabilityIdList on permission-changed; WireJsonObjectSchema for reply/error maps; `@public` tags; unknown-key strip + extra fixtures.
...
- Unit evidence: `omnicall-kit/evidence/SDK-03-transport-state-machine.md`
- Reviewer: `/sdk-review` **PASS** 2026-07-20 — independent re-run: `npx vitest run packages/sdk/src` (18), `npm run lint`, `npm run preflight` PASS; public `@softomnitel/omnicall-kit` surface empty; fake transport only; mutation non-replay + diagnostics redaction + leak proofs verified. Post-review Low fixes: FakeTransport excluded from pack; payload-free mutation counter.
...
- Interoperability matrix: `omnicall-kit/evidence/SDK-04-pairing-auth-capabilities.md`
- Security tests: `packages/sdk/src/public/auth-client.test.ts`, `packages/sdk/src/internal/pop-crypto.test.ts`
- Public API report: `omnicall-kit/etc/api/sdk.api.md` (auth lifecycle; no `OmniCallClient`)
...
> Implement the smallest useful read-only OmniCallClient API: lifecycle, snapshot,
...
- Unit evidence: `omnicall-kit/evidence/SDK-05-read-only-beta-api.md`
- API report: `omnicall-kit/etc/api/sdk.api.md` (read-only `OmniCallClient`, **37** symbols)
- Browser matrix: Chromium via `OMNICALL_SDK_BROWSER=1 npm run test:browser` (**4** tests)
- Desktop DI-05 evidence: `omnicall-kit-integration/evidence/DI-05-read-only-snapshot-events-window-show.md`
...
- Unit evidence: `omnicall-kit/evidence/SDK-07-operator-logout-workflows.md`
- Workflow tests: `packages/sdk/src/public/omnicall-client.operator.test.ts` (+ browser operator path)
...
- Unit evidence: `omnicall-kit/evidence/SDK-08-saved-profile-activation.md`
- Workflow tests: `packages/sdk/src/public/omnicall-client.activate.test.ts` (+ browser activate path)
...
- Desktop DI-08 evidence: `omnicall-kit-integration/evidence/DI-08-saved-profile-activation.md`
...
- Unit evidence: `omnicall-kit/evidence/SDK-09-developer-docs-examples.md`
