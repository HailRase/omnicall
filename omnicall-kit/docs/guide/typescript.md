...
How integrators should import and use types from `@softomnitel/omnicall-kit` without
...
Use `@softomnitel/omnicall-protocol` when you need:
...
client.subscribe('call:incoming', (event) => {
  // event: OmniCallEventOf<'call:incoming'>
...
```ts
type CampaignOffered = OmniCallEventOf<'operator:campaign-offered'>;
```
...
Keep `OmniCallClientError.details` as opaque `WireJsonObject` for forward
