...
| Requesting `window.hide` at pairing | Privileged — client sanitize always strips it | Grant via OmniCall Settings Origin matrix; then call `client.window.hide` |
...
if (!caps.includes('account.activate')) {
  // show host UI: ask operator to grant in OmniCall Settings
  return;
}
