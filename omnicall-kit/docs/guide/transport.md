...
The SDK speaks the OmniCall local protocol over a **byte channel**. It does **not** own
...
```ts
import { createBrowserWebSocketTransport } from '@softomnitel/omnicall-kit';
...
| Do not | Why |
| --- | --- |
| Put `originate` / `logout` on the socket wrapper | Product API lives on `OmniCallClient` |
