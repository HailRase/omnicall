# OmniCall Kit Architecture

## Decision

OmniCall Kit is a thin browser client over a versioned local protocol. It contains no
...
### `@softomnitel/omnicall-kit-testing`
...
- events: typed subscription by event name (`OmniCallEventOf`) and unsubscribe;
- integrator type re-exports: snapshot / capability / operator status DTOs from protocol
  (schemas and fixtures remain on `@softomnitel/omnicall-protocol`).
...
```text
application code -> @softomnitel/omnicall-kit -> @softomnitel/omnicall-protocol
test code        -> @softomnitel/omnicall-kit-testing -> @softomnitel/omnicall-protocol
```
