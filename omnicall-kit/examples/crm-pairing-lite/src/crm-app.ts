/**
 * Integrator-facing CRM helpers — public `@softomnitel/omnicall-kit` surface only.
...
export async function loadRevision(client: OmniCallClient): Promise<number> {
...
> {
  ...
  try {
    ...
    const code = isOmniCallClientError(error) ? error.code : undefined;
 ...
 */
export async function logoutDemo(client: OmniCallClient): Promise<LogoutOutcome> {
  const expectedRevision = await loadRevision(client);
  try {
