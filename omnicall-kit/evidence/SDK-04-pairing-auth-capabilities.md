...
Browser-side fail-closed auth client in `@softomnitel/omnicall-kit`:
...
6. Pre-auth snapshot/product events dropped; no product `OmniCallClient` methods
...
**Not exported:** `OmniCallClient`, `getSnapshot`, call/operator/account/window product methods.

`api:check` allowlist enforced in `scripts/api-check.mjs`.

## PoP interop vector

- Canonical string: `buildPopSigningPayload` from `@softomnitel/omnicall-protocol` (ADR-0016)
...
- Browser scaffold: `tests/browser/pop-crypto.browser.test.ts` (runs when `OMNICALL_SDK_BROWSER=1`)

## Verification commands (cwd `omnicall-kit/`)
...
| `npm run api:check` | PASS (sdk **28** symbols, no `OmniCallClient`) |
| `npm run package:check` | PASS (no publish; `auth-test-peer` / `fake-transport` excluded) |
| `OMNICALL_SDK_BROWSER=1 npm run test:browser` | PASS (**3** — PoP sign + IndexedDB round-trip) |
...
| `OMNICALL_SDK_BROWSER=1 npm run test:browser` | PASS (**3**) |
