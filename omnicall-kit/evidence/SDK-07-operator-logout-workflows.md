...
| Desktop DI-07 | `done` (`omnicall-kit-integration/evidence/DI-07-operator-logout-workflow.md`) |
| Scope | `omnicall-kit/` only — no desktop `src/` product edits in SDK unit |
...
- Typed failures via `OmniCallClientError` (incl. `details` for `interaction_required`, `currentRevision` on `stale_state`).
...
| browser | getReasons + interaction_required; no storage; disconnect does not extra-logout | `browser OmniCallClient operator getReasons + logout…` |
...
| Tests | `public/omnicall-client.operator.test.ts`, `tests/browser/omnicall-client-operator.browser.test.ts` |
