...
No OmniCallClient, no Zod install, no transport, no publish, no desktop `src/`, no DI-*
...
| PROTOCOL closed table | `omnicall-kit/docs/PROTOCOL.md` |
| Fixture format | `omnicall-kit/docs/COMPATIBILITY-FIXTURES.md` |
...
- No `@softomnitel/omnicall-protocol` schemas or fixtures content files yet
- No DI-01…DI-10 code
- SDK-02 not started

## Verification

Commands run from `omnicall-kit/`:
...
```text
cd omnicall-kit && npm run preflight
→ SDK preflight PASS
