# SDK-02 — `@softomnitel/omnicall-protocol` evidence
...
- Runtime dependency: `zod@4.4.3` (exact) in `@softomnitel/omnicall-protocol` only.
...
- No `OmniCallClient`, transport, crypto runtime, desktop `src/` product changes, or DI-* implementation.

## Public API surface (intentional)

Exports from `@softomnitel/omnicall-protocol`:
...
- **Not exported:** `OmniCallClient`, transport, WebSocket, IndexedDB/crypto client.

`api:check` updated: `@softomnitel/omnicall-kit` remains empty; `@softomnitel/omnicall-protocol` must export symbols and must not export `OmniCallClient`.
...
Commands (cwd `omnicall-kit`):

```bash
npm run build -w @softomnitel/omnicall-protocol
...
DI-01 is in **`review`**. Desktop now depends on workspace `@softomnitel/omnicall-protocol`
(`file:omnicall-kit/packages/protocol`) outside Domain and consumes
...
- Package: `omnicall-kit/packages/protocol/`
- Fixtures: `omnicall-kit/packages/protocol/fixtures/`
- API report: `omnicall-kit/etc/api/protocol.api.md`
- Dependencies: `omnicall-kit/docs/DEPENDENCIES.md`
- This file: `omnicall-kit/evidence/SDK-02-protocol-package.md`
