# SDK-05 Evidence — Read-Only Beta API (`OmniCallClient`)
...
1. **`createOmniCallClient`** — side-effect-free factory (no connect/pair/auth/snapshot).
...
| Tests | `public/omnicall-client.test.ts`, `omnicall-client.fixtures.test.ts`, `tests/browser/omnicall-client.browser.test.ts` |
...
npm run api:check     # PASS (sdk 37 symbols, createOmniCallClient present)
...
Oracle evidence: `omnicall-kit-integration/evidence/DI-05-read-only-snapshot-events-window-show.md`
