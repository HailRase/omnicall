...
Internal-only implementation inside `@softomnitel/omnicall-kit` (`packages/sdk/src/internal/`):
...
- No `OmniCallClient` product methods (calls/account/operator/window).
...
- No npm publish.
- Wire schemas unchanged (consume `@softomnitel/omnicall-protocol` only).

## Public API

`packages/sdk/src/index.ts` still `export {}`.

`api:check` — `@softomnitel/omnicall-kit` has **no** public production symbols; `@softomnitel/omnicall-protocol` unchanged (no `OmniCallClient`).
...
`/sdk-review` **PASS** 2026-07-20 — independent verification from `omnicall-kit/`:
