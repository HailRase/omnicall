...
Confirm whether a **public HTTPS** CRM page can reach OmniCall Desktop on
...
5. WebSocket constructors do **not** expose `targetAddressSpace`; connecting to the IP
   literal `ws://127.0.0.1:...` is the supported OmniCall approach (ADR-0015).
...
| --- | --- | --- |
| HTTPS page → `GET http://127.0.0.1:17341/omnicall/v1/discovery` | **Yes**, with LNA allow | Use `targetAddressSpace: "loopback"` when calling `fetch` |
| HTTPS page → `ws://127.0.0.1:17341/omnicall/v1/ws` | **Yes**, with LNA allow | Exact Origin still enforced by desktop (ADR-0011) |
...
- No `@softomnitel/omnicall-kit` transport client (SDK-03).
