...
| Desktop DI-08 | `done` (`omnicall-kit-integration/evidence/DI-08-saved-profile-activation.md`) |
| DI-10 | still `blocked` (SDK-09 open; no waiver) |
| Privileged security | ADR-0013 §B + ADR-0016 + ADR-AF-003/005/006 Accepted |
| Scope | `omnicall-kit/` only — no desktop `src/` product edits for this unit |
...
- Typed failures via `OmniCallClientError` (`forbidden`, `not_ready`, `timeout`, `stale_state`+`currentRevision`, `conflict`, `not_found`, `invalid_payload`).
...
| Browser | activate success; strip at pairing; no storage leak; disconnect non-activate | `browser OmniCallClient activateProfile success…` |
...
| Area | Paths |
| --- | --- |
| Public | `packages/sdk/src/public/omnicall-client.ts`, `omnicall-client-api.ts` |
...
| Tests | `public/omnicall-client.activate.test.ts`, `tests/browser/omnicall-client-activate.browser.test.ts` |
...
| `OMNICALL_SDK_BROWSER=1 npm run test:browser` | **7** passed (6 files) |
...
| `OMNICALL_SDK_BROWSER=1 npm run test:browser` | 7 | **7** | ±0 |
...
- Re-exports protocol budget constants from `@softomnitel/omnicall-kit`.
- Wire failure details (`activate_phase`, `failure_kind`) surface on `OmniCallClientError.details`.
