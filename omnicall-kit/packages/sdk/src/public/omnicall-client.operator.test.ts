...
describe('OmniCallClient operator/logout capability fortress', () => {
...
describe('OmniCallClient operator success and fail-closed', () => {
  ...
  it('SIP-only getReasons empty and changeStatus not_found', async () => {
    ...
    await expect(statusPending).rejects.toSatisfy(
      (error: unknown) =>
        isOmniCallClientError(error) && error.code === 'not_found'
    );
  });
...
describe('OmniCallClient account logout workflows', () => {
...
describe('OmniCallClient operator/logout reconnect and disconnect safety', () => {
  ...
  it('privacy: diagnostics never echo secret needles / destinations', async () => {
    ...
    await expect(pending).rejects.toSatisfy(
      (error: unknown) =>
        isOmniCallClientError(error) && error.code === 'interaction_required'
    );
...
describe('OmniCallClient operator events honesty', () => {
