...
| --- | --- |
| SDK-00…SDK-05 | `done` |
| Desktop DI-06 | `done` (`axatalk-sdk-integration/evidence/DI-06-call-command-router.md`) |
...
typed `CallMutationResult | AxatalkClientError` (incl. `currentRevision` on `stale_state`).
...
SDK-05 regressions remain in `axatalk-client.test.ts` (snapshot revision-bind, window, sequence gap) — green in suite.
...
| Tests | `public/axatalk-client.calls.test.ts`, `tests/browser/axatalk-client-calls.browser.test.ts` |
...
npm run api:check     # PASS (sdk 39 symbols; was 37; AxatalkCallsApi + CallMutationResult)
