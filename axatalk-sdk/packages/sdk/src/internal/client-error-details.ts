/**
 * Typed readers for common AxatalkClientError.details shapes.
...
export function isInteractionRequiredError(
  error: unknown
): error is AxatalkClientError & { readonly code: 'interaction_required' } {
  return isAxatalkClientError(error) && error.code === 'interaction_required';
}

/** @public */
export function isConflictError(
  error: unknown
): error is AxatalkClientError & { readonly code: 'conflict' } {
  return isAxatalkClientError(error) && error.code === 'conflict';
}

/** @public */
export function isOperationFailedError(
  error: unknown
): error is AxatalkClientError & { readonly code: 'operation_failed' } {
  return isAxatalkClientError(error) && error.code === 'operation_failed';
}
