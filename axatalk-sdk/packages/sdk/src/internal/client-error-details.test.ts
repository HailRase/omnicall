...
import { AxatalkClientError } from './client-errors.js';
import {
  isConflictError,
...
describe('client-error-details', () => {
  ...
  it('narrows error codes with typed guards', () => {
    const interaction = new AxatalkClientError({
      code: 'interaction_required',
      retryable: false,
      details: { requiresReason: true, reasons: [] }
    });
    const conflict = new AxatalkClientError({
      code: 'conflict',
      retryable: true,
      details: { failure_kind: 'not_in_post_call_processing' }
    });
    const failed = new AxatalkClientError({
