...
describe('AxatalkClient calls capability request', () => {
...
describe('AxatalkClient calls success paths', () => {
...
describe('AxatalkClient calls fail-closed and typed errors', () => {
  ...
  it('surfaces conflict and not_owner typed failures', async () => {
    ...
    await expect(ownerPending).rejects.toSatisfy(
      (error: unknown) =>
        isAxatalkClientError(error) && error.code === 'not_owner'
    );
  });
...
describe('AxatalkClient calls reconnect and disconnect safety', () => {
