...
function webStorageHasAxatalkKeys(storage: Storage): boolean {
  for (let index = 0; index < storage.length; index += 1) {
    const key = storage.key(index);
    if (key !== null && /axatalk|pop-key|pairing/i.test(key)) {
      return true;
    }
  }
  return false;
 ...
 * Axatalk material in `localStorage` / `sessionStorage` when those globals exist.
 */
export function detectDemoWebStorageUsage(
  ...
  if (local !== undefined && webStorageHasAxatalkKeys(local)) {
    return true;
  }
  if (session !== undefined && webStorageHasAxatalkKeys(session)) {
    return true;
  }
  return false;
}
