 ...
 * - Inputs: number value, disabled reasons, history recall callbacks, dial callbacks.
 * - Outputs: accessible dialpad UI without business logic.
 * @uiMeta lf=LF-020 f=F-003,F-013,F-016,F-021 smoke=R7-*
 */
export function Dialpad({
  ...
  const hasDigits = numberValue.trim().length > 0;
  const canDial =
          ...
          {KEYS.map(([key, sublabel]) => {
            ...
            return (
              ...
              >
                ...
                {sublabel !== null ? <span className={styles.keySub}>{sublabel}</span> : null}
              </button>
            );
          })}
        </div>
