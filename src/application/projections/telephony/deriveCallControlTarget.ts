...
export function deriveCallControlTarget(
  ...
  const { selectedCallId, lines, incomingCallId, incomingCallProjection, contacts } = input;

  if (selectedCallId !== null) {
    if (incomingCallId !== null && selectedCallId === incomingCallId) {
      return deriveIncomingCallControlLine({ projection: incomingCallProjection, contacts });
    }
