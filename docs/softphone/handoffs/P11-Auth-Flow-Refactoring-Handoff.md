...
- Sidebar: `SettingsSidebar` + `SettingsPanel` consume availability VM; disabled non-Account items + tooltip `settings.nav.disabled.authorizeFirst`; Axatalk SDK is a top-level leaf below Integrations (pre-auth reachable); Integrations/OCP remains gated until account session
