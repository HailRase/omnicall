...
- Out of scope WU4: real JsSIP/OCP adapters, E2E harness, full tray shell (P11).
...
| Step | Path |
| --- | --- |
| Use Case | `RetryConnectionUseCase` with channel `sip` / `ocp` / `both` |
...
| Ordered teardown | hangup → SIP unregister → OCP logout |
...
| Use Case | `ShutdownCleanupUseCase` — hangup, unregister, scheduler dispose, OCP logout |
...
- [x] App close → SIP/OCP cleanup without orphaned sessions (LF-079)
