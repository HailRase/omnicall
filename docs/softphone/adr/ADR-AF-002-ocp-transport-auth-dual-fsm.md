...
2. **Application owns reconnects that need a fresh HTTP token.** Before every new socket, Application calls `OcpProxyAuthenticatePort`, then supplies the fresh token to `ConnectOcpUseCase`. Ephemeral tokens live only in an in-memory, attempt-scoped Application object and are cleared on success, terminal failure, logout, or supersession.
   ...
   - **Progress failure reveal (amendment 2026-08-06):** `deriveOcpSignInProgressView` reveals any `failedExecutionStage` **immediately** with the real `failureKind` (no latent blue fill until stage timeout). Reconnect enables as soon as failure is projected. Modal-open progress also suppresses duplicate `authFeedback` toasts (same ownership idea as silent recovery).
   - **Six execution stages + timeout SSoT:** `requesting_authorization_token` → `submitting_token_to_ocp` → `awaiting_authorization_data` → `receiving_phone_credentials` → `connecting_sip_transport` → `authorizing_sip`. Per-stage ms live in `OCP_SIGN_IN_STAGE_TIMEOUT_MS` and must stay aligned with Application waiters (`OCP_AUTH_SESSION_TIMEOUT_MS`, `OCP_CREDENTIALS_TIMEOUT_MS` starts only after OCP authorized via `beginCredentialsTimeout`). Sum (+ slack) feeds `SDK_ACTIVATE_OCP_AUTH_BUDGET_MS`.
   - **Monotonic execution progress (amendment 2026-08-06):** `applyAuthorizationExecutionStage` never regresses to an earlier stage. Advancing to stage N marks stages `0…N-1` completed (so early `creds` during awaiting still complete `receiving_phone_credentials` when SIP starts). `enterCredentialsWait` must not overwrite an already-active SIP stage — only arm `beginCredentialsTimeout` (no-op if the waiter already settled). Fresh restart at the first stage clears a stale ready checklist.
   - **Recovery login ownership (amendment 2026-08-06):** Banner **Повторить** and System State **Повторить сервер** / **Переподключить** share `dispatchAccountRecoveryAction` → `executeOcpRecoveryRepeatSignIn`. Login for `/proxy/authenticate` resolves in order: explicit → `lastAuthorizationAttempt.login` → `authenticatedLogin` → SIP username; `connectOcp` stores attempt-scoped API key; SecretStorage is the durable fallback. Do **not** surface `LOGIN_REQUIRED` when a prior OCP login is still known. Stale Authorization `authorized` after Server `failed` still prefers **Retry server**.
