...
- Consequences: Application resolves templates and validates HTTPS before IPC; Domain stays Electron-free; F-030 round-trips `UserSettings.externalApplications` (schema **v16**, conditions.queueNames + windowBehavior); open history is profile-local (not F-030); call-ended lifecycle uses second typed IPC and runs before terminal-event opens.
