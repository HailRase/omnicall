...
The platform implements F-000 through F-015 on mock gateways (488+ tests). Production requires JsSIP, browser WebRTC media, and legacy operator WebSocket without breaking Domain, Application, CI, or mock-based development.
...
3. **Vertical slices** R1–R7 (registration → media → calls → controls → legacy operator platform → transfer deferred).
...
10. **legacy operator integration:** product-deferred per [ADR-0005](./ADR-0005-defer-legacy operator-plugin.md); see `../ADR-0005`. removed operator RAT / removed operator step smoke not an active gate.
