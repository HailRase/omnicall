...
**Prepared / next transport auth:** **F-011 / P12 External Host API + OmniCall Kit** — DI-00…DI-11 **`done`**; SDK-00…SDK-10 Mode A **`done`**; **ADR-0018** / **DI-11** `/sdk-review` **PASS** 2026-07-21 (boot hydrate + machine-common trust). **DI-05 follow-up (2026-07-27):** `window:hide` product-available (ADR-0013 amendment) — privileged Origin-matrix grant, telephony-busy deny, hide-only tray recovery, SDK `client.window.hide`. **DI-04/DI-09 follow-up (2026-07-27):** corrupt pairing `safeStorage` blobs self-heal (purge + re-pair); Settings IPC no longer throws `secret_load_failed` for SDK scope. **ADR-0021 (2026-07-27):** shared-desk call control (no ownership deny) + granular Origin-matrix `call.answer|reject|hangup|hold|mute` with umbrella `call.control` (DTMF); matrix write/load normalizes `call.control`=AND(granular); security/docs PASS-with-fixes remediated (SECURITY shared-desk section, CHANGELOG `1.1.0`, ADR-0012 supersession). Prior DI-05 notes (operator events / reserved / queueLabel / event-pipeline / DI-04 inbound serialization) still apply. F-011 stays **`in progress`** / P12 open. Next: remaining OPEN DI-10 smoke/waivers → F-011/`implemented` / P12 close.
