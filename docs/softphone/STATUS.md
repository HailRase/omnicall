...
**Updated:** 2026-07-27
**Version:** `1.0.0` (brand: **OmniCall** / SoftOmniTel; packages `@softomnitel/omnicall-kit` + `@softomnitel/omnicall-protocol`)
**Tests:** full root vitest **2728 passed** (1 skipped); omnicall-kit `npm run preflight` **PASS**; migration + preferences legacy import tests PASS
**Lint / typecheck:** `npm run lint` PASS · `npm run typecheck` PASS
...
**Auth Flow Refactoring / Hardening:** implementation + automated gate complete 2026-07-17 — independent account/OCP/SIP state, five-stage OCP progress, crash-safe saved profiles/secrets, one-click saved-profile entry, persistent auth errors and rolling 24-hour notification journal (F-029). Real staging OCP smoke SM-1…20 remains external verification. Version: `1.0.0`.
...
**Prepared / next transport auth:** **F-011 / P12 External Host API + OmniCall Kit** — DI-00…DI-11 **`done`**; SDK-00…SDK-10 Mode A **`done`**; **ADR-0018** / **DI-11** `/sdk-review` **PASS** 2026-07-21 (boot hydrate + machine-common trust). **DI-05 follow-up (2026-07-27):** `window:hide` product-available (ADR-0013 amendment) — privileged Origin-matrix grant, telephony-busy deny, hide-only tray recovery, SDK `client.window.hide`. Prior DI-05 notes (operator events / reserved / queueLabel / event-pipeline / DI-04 inbound serialization) still apply. F-011 stays **`in progress`** / P12 open. Next: remaining OPEN DI-10 smoke/waivers → F-011/`implemented` / P12 close.
