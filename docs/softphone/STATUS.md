...
**Prepared / architecture gate closed:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00 **`done`** (`/sdk-review` PASS; ADR-0009…0013, baseline `axatalk-sdk-integration/evidence/DI-00-baseline.md`, handoff `handoffs/P12-External-Host-API-Master-Handoff.md`). Feature status remains **`planned`** (no production implementation). Next: SDK-00 (`/sdk-project`), then SDK-01 open protocol rows; DI-01 only after DI-00 + SDK-01.
