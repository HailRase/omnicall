...
**Updated:** 2026-07-26
**Tests:** F-011 SDK event-pipeline hardening PASS (sequence/auth no false-gap; fan-out validate-before-bump; `sdk:server-shutdown`; focused vitest desktop + axatalk-sdk); F-028/F-011 OCP campaign→SDK + `queueLabel` / ADR-0020 prior PASS; F-028 UI call context PASS; F-016/LF-002 bootstrap splash PASS; F-030 preferences export/import PASS; prior operator-events + DI-06/07 shared-clock PASS
**Lint / typecheck:** focused vitest PASS for fan-out/product/SDK client; full-repo `tsc` may still have pre-existing unrelated errors
...
**Prepared / next transport auth:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00…DI-11 **`done`**; SDK-00…SDK-10 Mode A **`done`**; **ADR-0018** / **DI-11** `/sdk-review` **PASS** 2026-07-21 (boot hydrate + machine-common trust). **DI-05 follow-up (2026-07-23):** desktop emits `operator:status-changed` / `operator:session-changed` with coarse-advance revision — evidence `axatalk-sdk-integration/evidence/DI-05-operator-events-coarse-revision.md`. **DI-05/DI-07 follow-up (2026-07-23):** post-call reservation observability (`reservedTarget`/`reservedReasonId`, narrowed `kind`) — `axatalk-sdk-integration/evidence/DI-05-operator-reserved-observability.md`. **DI-05 follow-up (2026-07-26):** additive public `queueLabel` on `call:*` + snapshot from OCP ACD context (`OCP-Call-Context.md`; replaces legacy `OCPincomingCallProgress` CustomEvent path). **DI-05 follow-up (2026-07-26):** event-pipeline hardening — shared wire sequence accounting for auth lifecycle (no false gap), validate-before-bump fan-out, `sdk:server-shutdown` on controlled stop, publish-drop logs; docs ADR-0016/`events.md`/`OCP-Call-Context.md`. **DI-04 follow-up (2026-07-23):** per-connection inbound serialization so `sdk:auth-proof` + immediate `sdk:ping` cannot race — `sdkGatewayConnection.ts` / `localWsSessionInbound.ts`; evidence note in `DI-04-pairing-origin-capabilities.md`. F-011 stays **`in progress`** / P12 open. Next: remaining OPEN DI-10 smoke/waivers → F-011/`implemented` / P12 close.
