...
**Prepared / next transport auth:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00…DI-09 **`done`**; **DI-10 `blocked`** (hard-stop: SDK-06…SDK-09 still `pending`, no waiver — evidence `axatalk-sdk-integration/evidence/DI-10-blocker-sdk-prereqs.md`); SDK-00…SDK-05 **`done`**. Next: `/sdk-project` SDK-06 (or human waiver forbidding F-011 `implemented`). Feature status **`in progress`** (not `implemented`).
