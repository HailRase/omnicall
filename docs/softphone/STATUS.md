...
**Updated:** 2026-07-20
**Tests:** 2407 passed / 1 skipped (DI-05 **`done`** — `/sdk-review` PASS re-gate)
**Lint / typecheck:** lint + typecheck green; focused DI-05 60 + full `npm test` + `registry:check` 71/0 OK
...
**Prepared / next transport auth:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00…DI-05 **`done`**; next **DI-06** call command router (`/sdk-integration`); SDK-00…SDK-03 **`done`**; evidence `axatalk-sdk-integration/evidence/DI-05-read-only-snapshot-events-window-show.md`. Feature status **`in progress`** (not `implemented`).
