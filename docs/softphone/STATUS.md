...
**Updated:** 2026-07-27
**Tests:** F-011 SDK `window.hide` product path PASS (ADR-0013 amendment: matrix + telephony-busy deny + hide-only tray; focused vitest desktop + axatalk-sdk); prior F-011 event-pipeline / F-028 / splash / preferences PASS
**Lint / typecheck:** focused vitest PASS for window.hide / routing / Settings; full-repo `tsc` may still have pre-existing unrelated errors
...
**Auth Flow Refactoring / Hardening:** implementation + automated gate complete 2026-07-17 — independent account/OCP/SIP state, five-stage OCP progress, crash-safe saved profiles/secrets, one-click saved-profile entry, persistent auth errors and rolling 24-hour notification journal (F-029). Real staging OCP smoke SM-1…20 remains external verification. Version: `0.15.0`.
