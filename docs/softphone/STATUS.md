...
**Updated:** 2026-07-20
**Tests:** 2428 passed / 1 skipped (DI-06 **`done`** — call command router + revision-contract fix)
**Lint / typecheck:** lint + typecheck green; focused DI-06 set 81 + full `npm test` + `registry:check` 73/0 OK
...
**Prepared / next transport auth:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00…DI-06 **`done`**; next **DI-07** operator/logout via `/sdk-integration`; SDK-00…SDK-03 **`done`**; evidence `axatalk-sdk-integration/evidence/DI-06-call-command-router.md`. Feature status **`in progress`** (not `implemented`).
