...
**Prepared / architecture gate closed:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00 **`done`**; **SDK-00** workspace/CI **`done`**; **SDK-01** protocol/security ADRs ADR-0014…0017 **`done`**; **SDK-02** `@axatalk/protocol` **`done`** (`/sdk-review` PASS 2026-07-20; High/Low follow-up fixed; `axatalk-sdk/evidence/SDK-02-protocol-package.md`). Feature status remains **`planned`** (no desktop production gateway). Next: **SDK-03** (transport) and/or **DI-01** (consume same fixtures outside Domain).
