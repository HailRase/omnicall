...
**Prepared / architecture gate closed; contracts started:** **F-011 / P12 External Host API + Axatalk SDK** — DI-00 **`done`**; SDK-00…SDK-02 **`done`**; **DI-01** protocol ports/mocks/fixture consume **`done`** (`/sdk-review` PASS; `axatalk-sdk-integration/evidence/DI-01-protocol-ports-mocks.md`). Feature status **`in progress`** (contracts only — no WS/IPC/product gateway). Next: **DI-02** typed main-to-renderer broker (`/sdk-integration`).
