...
- Acceptance Criteria:
  ...
  - **Shell window layout (F-016):** compact anchors bottom-right; settings is 1000px centered and settings-only maximizable; close restores saved compact dimensions. Settings→compact is instant (0ms): no snapshot cover, bounds animation, or overlay close hold. Maximized close has no visible settings-min snap; compact remains non-resizable/non-maximizable.
...
- Implementation evidence (settings-only maximize **2026-07-30**): `resolveShellWindowMaximizable` / `resolveShellWindowMinimumSize`; `ShellWindowController.toggleMaximize` restores to settings min bounds (1000×session height); IPC `shell:window-toggle-maximize` / `shell:window-maximized-changed`; `ShellWindowControls` maximize on Win/Linux/macOS only when settings open; icons `shell.window.maximize` / `shell.window.restore`
 - Implementation evidence (instant settings close **2026-07-30**): `ShellWindowLayoutService` sends `0ms` for settings→compact; `SettingsFullscreenOverlay` unmounts immediately; `ShellWindowController` preserves compact restore and maximized no-snap behavior; service, controller, and overlay tests cover the contract.
