...
- Shell attention (2026-07-22): shared `bringBrowserWindowToFront` + `ShellWindowAttentionController`; raise on incoming/outgoing call (IPC), SDK Origin trust / pairing (main), activate consent; **root `SdkConnectCeremonyModal`** (TOFU → pairing stepper; pairing-only when Origin already allowed); **no** Settings auto-open for pairing; ADR-0013 / ADR-0018 §G / UX blueprint / TEST-MATRIX / SMOKE
