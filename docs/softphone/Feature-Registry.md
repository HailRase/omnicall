...
- Acceptance Criteria:
  ...
  - **SIP-only staged notifications (F-001/F-029, 2026-07-17):** SIP-only ready emits two success toasts — transport connected, then registration succeeded. `ok` + `telephony.registration_failed` never claims phone registered.
  - **Account feedback channel split (ADR-0026 amendment 2026-08-06):** validation → `AccountPanel` Alert; server/transport/REGISTER (incl. 403) → toast (`actionable`) with CTA «Состояние системы»; anti-dual (no Alert+toast); classifier `classifyAccountSignInErrorPresentation`.
- Test Coverage:
  ...
  - Application: `deriveAccountSignInNotificationFeedback.test.ts` (staged SIP success keys; classifier; notification vs inline channels)
  - Renderer: `useAccountActions.test.ts`, `useActionNotifications.test.ts` (no false SIP-ready success; System State CTA on notification toast; validation stays Alert)
...
- Implementation evidence (staged notifications + channel split): `AccountSignInOutcome.ts`, `deriveAccountSignInNotificationFeedback.ts`, `AccountBootstrapFacade.ts` (`sipTransportConnected`), `useAccountActions.ts`, `useActionNotifications.ts`, ADR-0026 amendment
...
- Acceptance Criteria:
  ...
  - **SIP preflight (2026-07-23):** `OutgoingCallOrchestrator` rejects when `TelephonyGateway.isRegistered()` is false **before** `OutgoingCallRequested` / hold-all — no Domain call events, no sticky UI card; PlatformError cause `sip_not_registered`.
  - Terminal mid-flight `CallFailed` projects to Idle + `lastOutgoingFailure`; UI shows a notification toast (not `OutgoingCallCard`). `OutgoingCallCard` is Connecting-only (pre-line).
...
- Acceptance Criteria:
  - No `window.Softphone` global API in OmniCall.
  ...
  - `window:hide` is product-available under privileged Origin-matrix grant + telephony-busy deny + tray Show recovery (ADR-0013 amended 2026-07-27).
- Test Coverage:
  - Unit: command payload parsing (F-028 E-12); DI-01 fixture corpus consume + mock gateway/broker + Domain dependency-boundary
  - Integration: DI-02 broker loopback; DI-03 loopback WS bind/limits/handshake/unauth deny/teardown; DI-04 Origin/pairing/PoP/capabilities/revoke; DI-05 redacted snapshot/events/`window:show`; DI-06 call commands via Call Engine (ownership/revision/idempotency); DI-07 operator/logout (`callType: "sdk"`, single-shot `account:logout` + `reasonId`, `interaction_required` without token); DI-08 saved-profile activate (`login` + Origin matrix `account.activate` + consent method picker; logout-first `conflict`); DI-09 Settings SDK Server card (origins/paired/revoke, hide disabled); **DI-11 `done`** (ADR-0018 trust store/blacklist/per-Origin matrix/always-on gateway/activate consent/boot hydrate; schema v11); **2026-08-03 Origin upgrade fail-closed** — only `allowed` Origins open WS (Trusted sites / seed); TOFU-on-upgrade superseded (ADR-0018 amendment); Settings UI (2026-07-21): three UI Kit Tabs (Main / Trusted sites / Blocked sites, Account-like slide indicator); trusted sites as UI Kit Accordion with FormField+Select permissions (allowed/denied) and explicit address Save/Cancel via UI Kit `IconButton` ghost; remove/blacklist confirms use AlertDialog `asChild` + UI Kit `Button` (ghost/destructive); blocked row origin+Unblock; shared Settings content measure (`SettingsForm.module.css`)
...
- DI-01 evidence: `omnicall-kit-integration/evidence/DI-01-protocol-ports-mocks.md`
- DI-02 evidence: `omnicall-kit-integration/evidence/DI-02-typed-main-renderer-broker.md`
- DI-03 evidence: `omnicall-kit-integration/evidence/DI-03-loopback-websocket-transport.md`
- DI-04 evidence: `omnicall-kit-integration/evidence/DI-04-pairing-origin-capabilities.md`
- DI-05 evidence: `omnicall-kit-integration/evidence/DI-05-read-only-snapshot-events-window-show.md`
- DI-05 follow-up (2026-07-23): operator public events + coarse-advance — `omnicall-kit-integration/evidence/DI-05-operator-events-coarse-revision.md`; mapper `ExternalSdkEventMapper` + `SdkOperatorEventRevisionGate`; bind via `bindSdkBrokerSession`
- DI-05 follow-up (2026-07-26): OCP ACD `queueLabel` additive on public `call:*` + snapshot call summaries (from `CallOcpContextResolved` / `CallOcpContextProjection`); no `acallid` on `call:*`; docs `OCP-Call-Context.md`, ADR-0017 O-OCP-1, `omnicall-kit/docs/guide/events.md`
- DI-05 follow-up (2026-07-26): campaign notify in protocol v1 — `operator:campaign-offered` / `cleared` + snapshot `operator.campaign` behind `operator.campaign.read` (ADR-0019 supersedes ADR-0017 O-CAMP-1 deferral); Domain `OperatorCampaignOffered` / `Cleared`; redacted DTO; no accept/reject commands; Origin matrix + i18n
- DI-05 follow-up (2026-07-26): campaign single-modal hold — second preview → `pendingPreview` (no SDK supersede); accept/reject/clear → Cleared then Offered; progressive while preview open updates badges only; snapshot `operator.campaign` = `activeCampaign ?? progressiveContext`
- DI-05 follow-up (2026-07-26): ADR-0020 — `call:acd-context` carries OCP MainCallIDInfo wire (`acallid`/`main_acallid`/parties) behind `ocp.acd_context.read`; Origin matrix + i18n; fan-out gate
- DI-05 follow-up (2026-07-26): ADR-0020 snapshot recovery — additive `calls[].acdContext` (snake_case MainCallIDInfo) from stored `acdWire`; gated by `ocp.acd_context.read`; `queueLabel` unchanged for `session.read.redacted`
- DI-05 follow-up (2026-07-26): event-pipeline hardening — auth lifecycle shares wire `sequence` without false `event.sequence_gap`; fan-out validate-before-bump; `sdk:server-shutdown` on `beginAppShutdown`/`stop`; publish drop observability (main IPC + renderer); docs sync ADR-0016 profiles + `events.md` / `OCP-Call-Context.md` reliability sections
- DI-05/DI-07 follow-up (2026-07-23): post-call reservation observability — optional `reservedTarget`/`reservedReasonId` on snapshot + `operator:status-changed`; `OperatorStatusChangeKind`; no separate reserve command — `omnicall-kit-integration/evidence/DI-05-operator-reserved-observability.md`; guide `omnicall-kit/docs/guide/operator-status-reservation.md`
...
- DI-04 follow-up (2026-07-27): pairing secret-load resilience — corrupt/undecryptable `sdk-gateway` blobs purged + treated as missing (`listPublic`/`findActive` never throw `secret_load_failed`); Electron safeStorage load purges bad files but still returns `secret_load_failed` for SIP scopes; Settings IPC returns `{ ok:false }` instead of unhandled reject; tests `sdkGatewayPairingStore.test.ts`, `ElectronSafeStorageSecretService.test.ts`; ADR-0011 Consequences
- Implementation evidence (DI-04 follow-up 2026-07-23): per-connection inbound serialization (`inboundTail` / `enqueueSdkGatewayInbound` in `sdkGatewayConnection.ts`; `localWsSessionInbound.ts`) so `sdk:auth-proof` + immediate `sdk:ping` cannot race; tests `sdkGatewayConnection.test.ts` + auth back-to-back ping; docs PROTOCOL/SECURITY/ADR-0016/TEST-MATRIX
- Implementation evidence (DI-05 read-only): `src/application/integration/ExternalSdkReadHandler.ts`; `ExternalSdkSnapshotAssembler.ts`; `ExternalSdkEventMapper.ts`; `SdkOperatorEventRevisionGate.ts`; `sdkPrivacyRedaction.ts`; `src/adapters/integration/sdkGatewayProductDispatch.ts`; `sdkGatewayWindowHandler.ts`; `sdkGatewayEventFanout.ts`; `LocalWsServerAdapter.product.test.ts`; `src/main/sdk/createSdkGatewayProductSurface.ts`
- DI-06 evidence: `omnicall-kit-integration/evidence/DI-06-call-command-router.md`
...
- Implementation evidence (ADR-0021 shared desk 2026-07-27): ownership gate removed for call control; granular caps `call.answer|reject|hangup|hold|mute` + umbrella `call.control`; Origin matrix rows + migration from umbrella; protocol `expandCallControlUmbrella` / `sessionHasCapability`; ADR `docs/softphone/adr/ADR-0021-sdk-shared-desk-call-control.md`
- Implementation evidence (ADR-0021 review remediation 2026-07-27): `normalizeSdkOriginCallMatrix` on parse/write; SECURITY shared-desk residual risk; CHANGELOG `[1.1.0]`; ADR-0012 ownership supersession note; WORK-UNITS race checklist without ownership-gate `not_owner`
- DI-06 follow-up (2026-07-23): SIP unregistered originate preflight — `OutgoingCallOrchestrator` + `TelephonyGateway.isRegistered()` before events; SDK reply `operation_failed` + `failure_kind: sip_not_registered` (no `call:outgoing`/`call:failed`); outbound `CallFailed` → Idle + toast (`lastOutgoingFailure`); `OutgoingCallCard` Connecting-only
- DI-07 evidence: `omnicall-kit-integration/evidence/DI-07-operator-logout-workflow.md`
- Implementation evidence (DI-07 operator/logout): `src/application/integration/ExternalSdkOperatorHandler.ts`; `ExternalSdkOperatorPort.ts`; `createSdkOperatorPortFromFacade.ts`; `mapSdkOperatorReasons.ts`; `externalSdkLogoutCommands.ts`; `ExternalSdkProductHandler.ts`; Facade `changeOcpStatusFromHost` `callType: "sdk"`; OCP wire map `sdk`→`external` via `mapOcpCallTypeToWire` / `buildOcpCommandPayload` (2026-07-23); `LocalWsServerAdapter.operator.test.ts`; `bindSdkBrokerSession.ts`
- DI-07 follow-up (2026-07-23): `operator:finish-appeal` → `FinishPostCallAppealUseCase` (`callType: "sdk"`); public status `post_call_processing`; OCP-login gate `not_found`; wrong status `conflict` + `failure_kind: not_in_post_call_processing`; ADR-0017 / PROTOCOL / guide synced
...
- DI-05 follow-up (2026-07-22): `window:show` local focus policy — raise above other apps (`show`/`focus`/`moveTop` + temporary always-on-top pulse restoring prior pin; rate limit preserved); `sdkGatewayWindowHandler.test.ts`; ADR-0013 / TEST-MATRIX / SMOKE aligned
- Shell attention (2026-07-22): shared `bringBrowserWindowToFront` + `ShellWindowAttentionController`; raise on incoming/outgoing call (IPC), SDK pairing (main), activate consent (`attentionId` per episode → IPC raise); **root `SdkConnectCeremonyModal`** (pairing when Origin already `allowed`; production Origin first-contact = Trusted sites / seed, not upgrade TOFU); **no** Settings auto-open for pairing; ADR-0013 / ADR-0018 §G / UX blueprint / TEST-MATRIX / SMOKE
- Connect ceremony UX (2026-07-22): `useSdkConnectCeremony` + `SdkConnectCeremonyModal` (blur overlay); removed Settings `SdkModuleSettingsAttentionSection` and `SdkOriginTrustConsentModal`
- Pending lifecycle (2026-07-22): deny pairing on disconnect; Origin leave-allowed → deny pending + close WS (no auto-revoke paired); TTL sweeper; waiting Cancel; Approve requires Origin still allowed — `sdkGatewayPendingLifecycle.test.ts`
- Login activate (2026-07-22): replace Settings temporary `profileRef` grant with `login` activate + consent method picker; matrix elevates `account.activate` + `sdk:permission-changed`; ADR-0013/0016/0018 updated
- Activate timeout sync (2026-07-23): consent TTL 120s + SIP/OCP auth budgets; broker/SDK hop ~240s for `account:activate-profile` only; additive `details.activate_phase` / `auth_mode` / `failure_kind`; OCP cancel on auth-budget expiry; ADR-0018 / PROTOCOL / guide aligned
- Operator modal deadline UI (2026-07-23): muted `MM:SS` header countdown on pairing / waiting / activate consent (legacy Origin-trust pending TTL retained); SSoT `sdkOperatorModalTimeouts.ts`; ADR-0018 §G.6
- Operator modal TTLs Settings (2026-07-23): Settings → OmniCall Kit → Main selects for consent / Origin-trust pending / pairing (`operatorModalTimeouts`); defaults unchanged (120s / 5m / 5m); live gateway + consent bridge via `applyPolicy`; activate hop ceiling `SDK_ACTIVATE_CLIENT_TIMEOUT_MS` = **420s** (max consent); ADR-0018 §G.7
- Activate inbound-queue release (2026-07-23): `account:activate-profile` detaches from per-connection inbound serialization while broker/consent pending so `sdk:ping` heartbeats succeed; fixes CRM bare `operation_failed` ~10s into consent wait; `releasesSdkInboundQueueWhilePending`; ADR-0018 §E.12 / PROTOCOL.md
...
- Acceptance Criteria:
  ...
  - **OCP unexpected-drop recovery UX (2026-07-30):** auto-recovery is capped (6×5s); in-flight `cancelAll(fresh_token_connect|sign_in_supersede)` must not reset the attempt budget; progress is `uiSurface: silent` (banner only — never open the six-stage sign-in Dialog); `transportRecoveryActive` keeps banner across disconnect/HTTP/WS flaps; exhaustion → Server `failed` + banner Retry.
- Test Coverage:
...
- Legacy IDs: `LF-002`, `LF-055`, `LF-056`, `LF-060`, `LF-076`, `LF-077`, `LF-082`, `LF-084`, `LF-085`, `LF-086`, `LF-087`, `LF-032` (multi-session toggle)
- Context: Settings
- Priority: high
...
- Acceptance Criteria:
  ...
  - **Settings nav groups (Integrations):** always-open cluster when expanded (muted sentence-case section label + flat child rows aligned with top-level items; no accordion, no nested inset card); collapsed rail keeps group icon only; gated children stay visible with soft disabled + authorize-first tooltip; OmniCall Kit remains top-level sibling (ADR-0018).
  ...
  - **Preferences transfer (F-030):** Settings → General export/import portable `omnicall.preferences` JSON (no secrets); see `P11-Operator-Preferences-Export-Design.md`.
  ...
  - **First-run sign-in CTA (2026-08-05):** when SIP is not registered, `CallIdleEmptyState` exposes a presentational CTA (`call-idle-sign-in-cta`, UI Kit `Button` `secondary`) under the needs-sign-in copy that opens Settings → Account via shell `openSettings("account")` (no Domain/SIP in components; light/dark via surface semantic tokens — not filled accent primary). Dialpad keeps disabled reason only (no duplicate CTA under Call). Settings Account shows a non-destructive first-run hint (`settings-account-first-run-hint`) when the saved-profile list is empty and the New tab is selected. Registered/session paths unchanged; `RegistrationStatusDot` remains status-only (no click).
  ...
  - **Feedback Channel Law (ADR-0026, amended 2026-08-06):** one presentational channel per feedback class — ephemeral outcomes via `notify`; Account **validation** → Alert + journaled `critical`; Account **server/register** → toast `actionable` + System State CTA (no Alert dual); confirm modals without outcome strips; multi-call policy error only in `CallSessionStack`; banners/modals/rich results stay non-toast. Continuation: SDK/OCP/EA/ES save mutations + screen-share confirm → toast; codec policy / list-load / ES RunResult keep owning surfaces.
...
- Test Coverage:
  ...
  - Component: `SettingsPanel`, `SettingsFullscreenOverlay`, `SettingsSidebar`, section panels; `UserAvatar`, `RegistrationStatusDot`, `SoftphoneShellHeader`; `IconTooltip.test.tsx` (T-001); `BootstrapSplashShell.test.tsx` (LF-002 boot splash); Storybook layout + settings overlay (WU0+) + `BootstrapSplashShell` light/dark
...
- Implementation evidence (first-run sign-in CTA **2026-08-05**): `CallIdleEmptyState` CTA; shell wiring `SoftphoneReadyShell` → `CallContextShell` → `openSettings("account")`; `SettingsAccountPanel` empty-profile hint; tests `CallIdleEmptyState.test.tsx`, `SettingsAccountPanel.test.tsx`; Storybook light/dark; i18n `call.idle.needsSignIn.*`, `account.firstRun.hint.*` (ru/en/fr/de/bg); guides `User-Guide-RU.md` §3, `install-instruction.md` §4
...
- Implementation evidence (Step 7 composition): `createRealAccountBootstrap.ts`, `createRealBootstrapSettingsRepository.ts`, `resolveOmniCallProfilesStorageRoot.ts`, `registerProfilesPersistenceIpc.ts`, `PreloadFileSystemAdapter.ts`, `createRealAccountBootstrap.test.ts`, `resolveRealBootstrapDiskOptions.ts`
...
- Related: **F-016** (settings UX), **F-001** (SIP authorize), extends **LF-077** stub from WU4; **F-031** External Services reuses F-023 profile keys for config (`UserSettings.externalServices`) and per-profile journal files under `profiles/external-services-journal/` without sharing buckets across identities
...
- Acceptance Criteria:
  ...
  - Successful feedback distinguishes SIP registration from OCP+SIP readiness; failed **server/register** authorization feedback offers navigation to System State via toast action (validation stays on Account Alert; ADR-0026 amendment 2026-08-06).
- Test Coverage:
  ...
  - Component: `SavedAccountProfileSelector`, `DeleteSavedAccountProfileConfirmationModal`, `OverwriteSavedAccountCredentialsConfirmationModal` (Cancel + ButtonGroup split continue/overwrite; confirm closes before `signInAccount` settles), `SettingsAccountPanel`, `AccountPanel` (SIP/OCP mode tabs, dual-status recovery, no Account logout/switch/generic retry, startup registration CTA)
  - Bootstrap: `createRealAccountBootstrap.test.ts`, mock repository injection

  - E2E: deferred
- Implementation evidence: `AccountBootstrapFacade.ts` (`forgetRememberedSipPassword`, `getActiveSipAccount`), `deriveSavedProfileCredentialPromptState.ts`, `mapAccountAuthorizationError.ts`, `SavedAccountProfileSelector.tsx`, `AccountPanel.tsx`, `useAccountActions.ts` (`confirmOverwriteExistingCredentials` closes modal then `handleSubmit(true, true)`, T-037), `OverwriteSavedAccountCredentialsConfirmationModal.tsx` (Cancel + ButtonGroup continue/menu-overwrite), `useSettingsActions.ts`, `SettingsAccountPanel.tsx`, `createMockAccountBootstrap.ts`, `messages.ts` (ru/en/fr/de/bg)
...
- Acceptance Criteria:
  ...
  - **OCP Module edit-only after account session (ADR-AF-003/005):** no first-time Connect/Disconnect/login picker/generic Retry; active-profile configuration only (enabled, autoConnect, domain); `Ocp-Proxy-Api-Key` is not edited in Integrations (acquired during authorization); Server/Authorization status owned by System State OCP tab.
  ...
  - **OCP sign-in progress overlay (global):** Account Login, modal Reconnect, or SDK `account:activate-profile` (OCP mode) with `authorizationProgress.uiSurface === "modal"` opens the same blurred Dialog with **six** per-stage timed Progress bars (`OCP_SIGN_IN_STAGE_TIMEOUT_MS` SSoT; fill only while in flight; green complete / red failed or timeout), per-status Lucide icons (animated when available; active uses centered CSS spin; timeout uses hourglass), failure IconTooltip with **real** `failureKind`/`failureCode` (no latent blue wait, no rewrite to timeout), footer **Disconnect OCP** (outline while in progress / destructive on failure; `cancelOcpSignInAttempt` → pre-login idle) and **Reconnect** (enabled as soon as `failedExecutionStage` is set). Mounted once in `SoftphoneReadyShell` / `AuthAccountShell` (not inside Settings `AccountPanel`); visible on dialpad, contacts, history, and settings. Density: `comfortable` while settings overlay is open (~440px), `compact` on the main compact window (smaller type/gaps/icons, Dialog `sm`). Open gate: `shouldOpenOcpSignInProgressModal` requires `uiSurface: modal` plus in-flight/failure view (plus optimistic open on manual OCP Login / modal Reconnect). While the Dialog owns the attempt, shell suppresses duplicate `authFeedback` toasts.
  - **Background OCP transport recovery (not the Dialog):** unexpected socket drop → `OcpTransportRecoveryService` with `uiSurface: silent` + global `OcpConnectionBanner` in the shell overlay layer (`--z-shell-status-banner`, above Settings/routes, below Dialogs); does not open `OcpSignInProgress`. Banner is a compact one-line chip (`OCP · status` + outline Retry on failed) sized for compact main display — not a Sonner toast. New attempts clear prior completed stage checklists; active stage wins over stale completed in `deriveOcpSignInProgressView`.
  - **Modal Reconnect (single Application owner):** calls Facade `recoverOcpSignInFromModal` — resolves strategy via `resolveAuthorizationRetryStrategy` (same matrix as `retryAuthorization` / dual-FSM / System State), seeds the first visible execution stage immediately, reuses `lastAuthorizationAttempt` + attempt-scoped secrets, **never** calls `signInAccount` or the new-login identity gate. Works while `hasActiveAccountSession === true` (ADR-AF-005).
  - **Modal Disconnect OCP (`cancelOcpSignInAttempt`):** cancels active attempt + credentials apply epoch, disarms transport recovery, disconnects OCP socket, resets Server/Auth/operator/campaign/progress to cold idle, then **`EndUserSessionUseCase`** so Login re-enables without a separate avatar Logout. Distinct from Settings OCP-only disconnect (keeps SIP/account session).
  - **Account panel `reconnect` recovery:** shown only when the selected saved profile owns the active account session (`activeSessionProfileId`); other profiles must not show «Переподключить» while OCP is authorized.
  - **Attempt ownership / supersession:** orchestration run identity + SIP apply epoch checkpoints; cancelled/superseded runs must not mutate active progress or start promote/register after cancel (non-cancellable boundary: promote already resolved ok — ADR-AF-005 session kept; register skipped if cancelled before register start).
  ...
  - Settings OCP-only disconnect keeps SIP; modal Disconnect and avatar Logout cascade SIP via `EndUserSessionUseCase`.
  ...
  - `callType: internal | external | sdk` on status-change commands for Application audit trail; OCP wire `function_call_type` is only `internal`|`external` (`mapOcpCallTypeToWire`: `sdk`→`external`).
...
- Test Coverage:
  ...
  - Renderer: **WU-04 done** — `AccountPanel` / `SettingsAccountPanel` mode tabs + OCP InputGroup; no Account logout/switch/generic retry; **WU-05 done** — Settings pre-auth gate + edit-only OCP Module; **T-034 done** — System State SIP/OCP tabs own Server/Authorization; Account keeps in-progress recovery actions only; **OCP progress overlay** — shell-mounted `OcpSignInProgress` Dialog + live store progress + Disconnect OCP / Reconnect; opens from progress for SDK activate; density compact/comfortable (compact = icons-only stage status + failure tooltip); modal Reconnect → `recoverOcpSignInFromModal` (not `signInAccount`); a11y live status on semantic stage changes
...
- Implementation evidence (OCP progress modal 2026-07-19): `deriveOcpSignInProgressView`; projection `stageStartedAtMs`/`failureKind`/`failureCode`; Facade `cancelOcpSignInAttempt`; `OcpSignInProgress` Dialog modal + UI Kit Progress; live store progress in `useAccountActions`; i18n failure tooltips ru/en/fr/de/bg
- Implementation evidence (OCP modal recovery harden 2026-07-19): Facade `recoverOcpSignInFromModal` + `resolveOcpRecoveryFirstVisibleStage`; orchestration run ownership; `OcpSipCredentialService` apply-epoch checkpoints; `useAccountActions` Reconnect without `signInAccount`; i18n Disconnect OCP; tests in orchestration/credential/facade/hook/modal suites
- Implementation evidence (modal Disconnect + scoped Account reconnect 2026-07-19): `cancelOcpSignInAttempt` → OCP idle + `EndUserSessionUseCase` (Login unlock); `deriveAccountSignInViewModel` / `deriveAllowedAccountRecoveryActions` gate `reconnect` by `activeSessionProfileId`; tests `AccountBootstrapFacade.accountSignIn.test.ts`, `accountSignInViewModel.test.ts`
- Implementation evidence (global OCP progress overlay 2026-07-22): shell-mounted `OcpSignInProgress` in `SoftphoneReadyShell` / `AuthAccountShell` (removed from `AccountPanel`); `shouldOpenOcpSignInProgressModal` auto-opens on live progress (SDK activate + manual); density `compact`|`comfortable`; tests `shouldOpenOcpSignInProgressModal.test.ts`, `OcpSignInProgress.test.tsx`, `useAccountActions.test.ts`
- Implementation evidence (OCP transport recovery UX harden 2026-07-30): `authorizationProgress.uiSurface` modal|silent; `OcpTransportRecoveryService` preserves attempt budget during in-flight connect disarm, seeds silent progress, clears on success/exhaustion, `markTransportRecoveryExhausted`; `preparing` clears completed stages; `deriveOcpSignInProgressView` active-over-completed; Facade `connectOcp({ progressUiSurface: "silent" })` for auto-recovery; tests recovery/progress/modal/hook suites; ADR-AF-002 amendment
- Implementation evidence (compact progress + activate consent UX 2026-07-22): compact stage status icons-only; `SdkActivateProfileConsentModal` Cancel▾+Allow; Deny awaits `account.activate=false` persist + `notifySdkIntegrationSettingsChanged` so Settings Trusted matrix refreshes (`useSdkSettingsPanel.refreshSnapshot` syncs origins)
...
- Implementation evidence (OCP connection banner global overlay 2026-07-30): `OcpConnectionBanner` moved from header slot into `SoftphoneReadyShell` overlays (alongside `OcpSignInProgress`); `--z-shell-status-banner`; visible over Settings/routes; tests `OcpStatusChrome.test.tsx`
- Implementation evidence (OCP connection banner compact chip 2026-08-06): one-line `OCP · status`; viewport-edge geometry (no titlebar horizontal safe-inset width clip on ≥360px); failed Retry = full-width `outline` + danger tokens (not `primary`); channel remains shell banner (ADR-0026); tests `OcpStatusChrome.test.tsx`; canon `UI-Design-System.md` § OCP connection banner
- Implementation evidence (OCP progress honesty 2026-08-06): immediate failure reveal in `deriveOcpSignInProgressView` (removed latent timeout fill); stage `receiving_phone_credentials` + `beginCredentialsTimeout` after OCP authorized; `awaiting_authorization_data` bar 15s; timeout visual state; modal suppresses `authFeedback` toasts; ADR-AF-002 amendment; tests derive/modal/credential/projection; i18n `stage.receiveCredentials` ru/en/fr/de/bg
- Implementation evidence (OCP progress monotonic stages 2026-08-06): `applyAuthorizationExecutionStage` prefix-complete + no regress; `enterCredentialsWait` skips UI when SIP already active (early `creds`); tests projection + orchestration early-creds gate; ADR-AF-002 amendment
- Implementation evidence (OCP recovery login 2026-08-06): banner Retry → `dispatchAccountRecoveryAction("retry_server")`; `resolveOcpLogin` cascade (attempt → authenticatedLogin → SIP); `connectOcp` stores attempt API key; SecretStorage fallback in `executeOcpRecoveryRepeatSignIn`; dual-FSM primary Retry server when Server failed with stale authorized; tests Facade reconnect-without-SIP + operator-status selector; ADR-AF-002 amendment
...
- Implementation evidence (Settings Integrations always-open cluster 2026-07-30): `SettingsSidebar` section label + flat children (no accordion/inset card); soft disabled + tooltip; collapsed rail group icon; tests in `SettingsSidebar.test.tsx`; UI canon `UI-Design-System.md` § Settings Nav Groups
...
- Implementation evidence (E-09 shell raise 2026-07-26): preview campaign → `useShellWindowAttentionFromCampaign` → IPC `shell:window-raise` (`ocp_campaign_offer`, dedupe campaign id) → `ShellWindowAttentionController` / `bringBrowserWindowToFront` (ADR-0013); progressive does not raise; tests hook + contract parse
- Implementation evidence (E-09/E-10 call context 2026-07-26): `CallOcpContextProjection` + bridge queue resolve (empty queue = internal/direct hide); `campaignEventProjection` progressive gate (`activeCampaign` preview modal vs `progressiveContext` badges); `deriveCallContextBadges` + `CallContextBadges` on Incoming overlay/session card + `CallSessionCard`; compact center modal with blur scrim; parse allows empty `queue`; docs `OCP-Call-Context.md`; LF-037…040 parity; tests projection/bridge/modal/i18n
- Implementation evidence (E-09 campaign FSM 2026-07-26): `CampaignEventProjection` phase `idle|preview_offered|progressive_offered` + `pendingPreview` hold; second preview does not supersede modal; clear/promote emits Cleared then Offered; progressive under preview = badges only (no SDK Offered); docs `OCP-Call-Context.md` / ADR-0019; tests `campaignEventProjection` / lifecycle
- Implementation evidence (E-10 / F-011 SDK queueLabel 2026-07-26): Domain `CallOcpContextResolved`; bridge publishes on non-empty queue; `ExternalSdkEventMapper` / snapshot assembler additive `queueLabel`; protocol optional field on call events + `SnapshotCallSummary`; no DOM CustomEvent; tests mapper/assembler/bridge
- Implementation evidence (E-10 / F-011 SDK campaign 2026-07-26): ADR-0019; protocol events + `operator.campaign.read`; Domain `OperatorCampaignOffered` / `Cleared`; lifecycle publish; mapper/snapshot/fan-out capability gate; Origin matrix migration; tests + fixtures
- Implementation evidence (E-10 logic): `OcpTelephonyBridgeService` call lifecycle (`get_main_acallid` / `dlg_stop` + correlation map + call-context/campaign cleanup); `selectIsCallButtonBlocked`; `OcpTelephonyBridge.integration.test.ts`; i18n keys `ocp.dialpad.reservedToCall`, `ocp.incomingCall.rejectWithBreakReason`
- Implementation evidence (E-10 dlg_stop single-shot 2026-07-26): wire `{ acallid: sipCallId }` only; bridge dedupe + `CallRejected`/`CallRejectedByDnd`; hangup/session-ended race skips duplicate `CallEnded`; local cancel skips trailing `CallFailed`; docs `OCP-Call-Context.md`; tests bridge/hangup/payload
- Implementation evidence (E-10 wire fix 2026-07-26): `get_main_acallid` outbound maps SIP `callId` → wire `acallid` + `user_login` + `caller_id`/`called_id` + `event`; inbound MainCallIDInfo accepts live `acallid`/`main_acallid` aliases; SDK `call:acd-context` + queue badge ellipsis/tooltip; docs `OCP-Call-Context.md`; adapter/bridge/mapper/UI tests
- Implementation evidence (E-10 / F-011 ADR-0020 2026-07-26): `call:acd-context` payload = full MainCallIDInfo snake_case + `callId`; capability `ocp.acd_context.read`; Domain `ocp` block; fan-out/matrix/i18n; fixtures + mapper/bridge tests
- Implementation evidence (E-10 / F-011 ADR-0020 snapshot 2026-07-26): `CallOcpContextProjection.acdWire` → snapshot `calls[].acdContext`; strip without `ocp.acd_context.read`; assembler/gateway tests
...
- Implementation evidence (T-027 status UX 2026-07-14): FSM Ready/Break/Preparing idle targets; `resolveOperatorStatusChangeMode`; `ChangeOperatorStatusUseCase` intent `auto|apply|reserve` + reserved outcome/event; selector stays enabled when busy; reservation toast; post-call modal (superseded by T-048)
- Implementation evidence (T-048 post-call finish 2026-07-19): PCP mode=`reserve`; `FinishPostCallAppealUseCase`; reserved clear on idle; dropdown finish footer; i18n `ocp.postCall.finishAppeal`; removed `OcpPostCallStatusModal`
...
- Implementation evidence (T-046 follow-up current option chrome 2026-07-19): current Ready green / Break orange (`--color-status-online` / `--color-status-dnd`) border+text only; current inert; hover same accents; idle `isCurrent` when `status` is READY|BREAK and `reasonId` equals option id (Preparing/unmatched → no active)
- Implementation evidence (T-046/T-048 reserved booking chrome 2026-07-23): `resolveOperatorStatusOptionIsCurrent` — busy/PCP with `reservedStatus`+`reservedReasonId` marks that option `isCurrent` in `OperatorStatusSelector` / `OcpStatusDropdown` (footer finish label unchanged); stories `PostCallFinishAppeal*`; tests `operatorStatusPresentation.test.ts`, `useOperatorStatusSelector.test.ts`; SDK guide `operator-status-reservation.md` desktop UI + finish default-Ready note
...
- Implementation evidence (ADR-0018 OmniCall Kit nav, 2026-07-21): top-level `integrations-sdk` leaf below Integrations group; pre-auth SDK enabled, Integrations/OCP gated (`settingsSections.ts`, `SettingsSidebar.test.tsx`)
...
- Test Coverage:
  ...
  - SIP-only sign-in success is two journaled toasts (`account.success.sipTransportConnected`, `account.success.sipRegistrationSucceeded`); SIP connect/register errors are toast-channel (`actionable`) with action `account.notification.openSystemStateAction` («Состояние системы»); validation-class Account errors journal with `critical` (no toast) while Alert owns UX (ADR-0026 amendment 2026-08-06).
...
- Test Coverage:
  ...
  - Integration: `OperatorPreferencesExternalServices.integration.test.ts` (F-031 runtime refresh + fail-closed)
...
- Acceptance Criteria:
  ...
  - Journal caps at 100; response body ≤16 KiB; Authorization/Cookie/X-Api-Key values persist/display as `***`.
  - Profile switch/logout cancels pending/waiting old-profile jobs; in-flight may finish to the captured journal; F-030 round-trips config (journal excluded).
  - Empty default is inert; SIP-only bootstrap unchanged; no F-011/F-028/SIP/headset/transfer behavior mutation.
- Test Coverage:
  - Unit: WU-01 settings parse/migrate; WU-02 HTTP/journal/file mock contracts; WU-03 matcher/template/request/security + event mapper; WU-04 queue FIFO/max-3, execute classification/redaction/journal isolation, manual Run now, IPC parsers, main redirect/timeout; WU-05 file journal parse/cap/corrupt, Save/Query Use Cases; WU-06 PreferencesExportDocument + OperatorPreferences Use Cases External Services round-trip; WU-07 ExternalServiceCollectionDocument + Export/Import Use Cases + collection-file IPC parsers; WU-08 collection mutation helpers + collections panel projection; WU-09 request mutation helpers; WU-10 journal panel projection cap/redaction/truncation; WU-13 delay scheduler + v12→v13 trigger migration
  - Integration: WU-04 non-blocking `handleCommittedEvent` + composition wiring; WU-05 profile A/B isolation, logout pending cancel, in-flight old-journal completion, AccountSessionActivated/UserSessionEnded, failed-draft non-promotion; WU-06 F-030 External Services round-trip + runtime refresh; WU-07 facade collection import/export + cancel/fail-closed; WU-11 committed focus reducer + post-store binder
  - Component / Storybook: WU-08 Postman-like sidebar tree + workspace (`ExternalServicesSidebar`, `ExternalServicesPanel`, Settings Integrations leaf, light/dark stories); WU-09 request URL bar/tabs/Send/Response (`ExternalServicesRequestsEditor.test.tsx`, light/dark stories); WU-10 journal History tab (`ExternalServicesJournal.test.tsx`, light/dark stories); WU-13 Queue tab + trigger delay controls; UI polish 2026-07-30 without feature downgrade
  - E2E: deferred; product acceptance closed in `external-services-plan/11-ACCEPTANCE.md` (WU-12)
- Design / plan: `external-services-plan/README.md` (spec `00-PRODUCT-SPEC.md`, architecture `01-ARCHITECTURE.md`, WUs `10-WORK-UNITS.md`)
- ADR: `docs/softphone/adr/ADR-0022-external-services-http-isolation.md` (**Accepted**); `docs/softphone/adr/ADR-0023-external-services-per-trigger-delay.md` (**Accepted**)
- Handoff: `docs/softphone/handoffs/P14-External-Services-Master-Handoff.md`
- Progress: `external-services-plan/PROGRESS.md`
- Implementation evidence (WU-04): `ExternalServicesDispatchQueue`, `ExternalServicesRuntimeRegistry`, `ExternalServicesAutomationService`, `ExecuteExternalServiceRequestUseCase`, `RunExternalServiceRequestNowUseCase`, `ExternalServicesComposition`, `ExternalServicesHttpContract`, `PreloadOutboundHttpAdapter`, `registerExternalServicesHttpIpc` / `executeExternalServicesHttpRequest`; synthetic `AccountBootstrapFacade.handleExternalServicesCommittedEvent`
- Implementation evidence (WU-05): `externalServicesJournalDocument.ts`, `FileExternalServicesJournalRepository.ts`, `profileStoragePaths.ts` journal paths; `SaveExternalServicesSettingsUseCase`, `QueryExternalServicesUseCase`; composition `activateProfile` / `replaceActiveSettings` / `invalidateLifecycle`; facade `AccountSessionActivated`/`UserSessionEnded` + `saveExternalServicesSettings` / `queryExternalServices`; real bootstrap file journal injection; cross-ref F-023 profile isolation
- Implementation evidence (WU-06): F-030 `PreferencesExportDocument` v12 External Services round-trip (journal excluded; authored header/query values portable); facade `importOperatorPreferences` → `replaceActiveSettings`; `OperatorPreferencesUseCases` + `OperatorPreferencesExternalServices.integration.test.ts`; design `P11-Operator-Preferences-Export-Design.md`; cross-ref F-030
- Implementation evidence (WU-07): ExternalServiceCollectionDocument (omnicall.external-service-collection v1); ExportExternalServiceCollectionUseCase / ImportExternalServiceCollectionUseCase (ID regenerate + (copy N)); ExternalServicesCollectionFileContract + preload/main IPC (JSON, UTF-8, 2 MiB); facade importExternalServiceCollection / exportExternalServiceCollection; composition wiring; focused + facade integration tests
- Implementation evidence (WU-08): Settings leaf `integrations-external-services` beside OCP (SDK top-level preserved); icon `settings.integrations.external-services`; Application collection summaries + mutation helpers; hooks `useExternalServicesShell` / `useExternalServicesActions` / `useExternalServicesPanel`; Postman-like sidebar tree + workspace (2026-07-30 polish); i18n `settings.integrations.externalServices.*` ru/en/fr/de/bg; navigation/panel/component tests + light/dark stories; cross-ref F-016
- Implementation evidence (WU-09): request workspace with method/URL/Send bar, Params/Headers/Body/Triggers tabs, save/delete/discard confirmation; `useExternalServicesRequestActions` persists drafts before facade Run now and exposes queued/running/classified feedback in Response pane; `ExternalServicesRequestsEditor.test.tsx`; `ExternalServicesRequestsEditor.stories.tsx`; locale updates ru/en/fr/de/bg.
- Implementation evidence (WU-10): `deriveExternalServicesJournalPanel`; `useExternalServicesJournal`; `ExternalServicesJournal` / `ExternalServicesJournalEntry` / `ExternalServicesJournalEntryDetail` in Response pane History tab (newest 100, already-redacted headers as `***`, truncation marker, empty/loading/error/retry); i18n `settings.integrations.externalServices.journal.*` + `trigger.manual_run` ru/en/fr/de/bg; `ExternalServicesJournal.test.tsx` + light/dark stories.
- Implementation evidence (journal summary polish 2026-07-30): collapsed History row shows colored HTTP method badge plus vertical separators between method / trigger / outcome / status / duration without growing row height; journal entry persists `method`; Variables catalog group `always` label → «Общие» / General / Général / Allgemein / Общи.
- Implementation evidence (WU-11): `callFocusProjection` becomes the Application source for focused lines; `useAccountBootstrapStore` commits it with call projections; `readExternalServicesProductStateFromStore` exposes a typed structural snapshot; `bindExternalServicesAutomation` registers after `bindFacade`, never awaits HTTP, and disposes with bootstrap; focus/reader/binder/non-interference tests cover call, campaign, and ACD facts.
- Implementation evidence (WU-13): `UserSettings` schema **v13** trigger bindings `{ eventType, delaySeconds }`; `ExternalServicesDelayScheduler`; Queue tab + cancel; logout delayed-jobs warning; ADR-0023; migration + scheduler tests.
- Implementation evidence (WU-12): acceptance/docs closeout; v13 fixture/lint remediation; full gates (`test`/`typecheck`/`lint`/`i18n:check`/`ui:catalog`/`registry:check`) PASS; SemVer MINOR shipped **1.2.0** (2026-07-31).
- Implementation evidence (panel hook split 2026-07-30): `useExternalServicesPanel` orchestrator + `externalServicesPanel/*` (selection/queue/dialogs/workspace + sidebar/editor builders); acceptance file-budget residual cleared.
- Implementation evidence (UI polish 2026-07-30): Postman-like workspace shell (sidebar COLLECTIONS tree + main editor + Response/History) without removing collections/requests/run/journal/variables capabilities; docs `external-services-plan/05-UI-UX.md` upgraded (not downgraded); risk note clarifies layout-only vs Postman clone.
- Implementation evidence (UI polish density 2026-07-30): centered empty states; History without duplicate journal heading; no request-tree indent; breadcrumb click-to-edit rename; fixed sidebar request rename dialog; collection quick-add `+`; Params/Triggers count badges; compact KV + journal rows; body-mode radios; Response/History expand/collapse; sidebar header hover; icons `external-services.add` / `panelExpand` / `panelCollapse`.
- Implementation evidence (variables discoverability 2026-07-30): Domain `ExternalServiceVariableCatalog` SSoT; request editor Variables tab + Insert into URL/Body; collection variables dialog clarifies system precedence; i18n `settings.integrations.externalServices.variables.*` ru/en/fr/de/bg; tests `ExternalServiceVariableCatalog.test.ts`, `ExternalServicesRequestsEditor.test.tsx`; docs `03-EVENTS-AND-VARIABLES.md` / `05-UI-UX.md` / `11-ACCEPTANCE.md` synced.
...
- Implementation evidence (query/journal isolation + load UX 2026-08-03): `QueryExternalServicesUseCase` adds `journalStatus` (`ready`/`skipped`/`error`); `journalLimit: 0` skips journal I/O; journal repository failure keeps settings `ok`; shell/journal hooks honor status; workspace top banner (`external-services-load-error`) instead of flex-breaking Alert; History compact retry; tests Use Case + `ExternalServicesPanel.test.tsx`; docs `01-ARCHITECTURE` / `05-UI-UX` / `06-PERSISTENCE-EXPORT` / `12-RISKS` synced; CHANGELOG Unreleased.
...
- Acceptance Criteria:
  ...
  - **Geometry editor UI** (General tab, bottom, `electron_window` only): **centered** stage with **adaptive preview scale** (divisor 4–8 from stage content width via `ResizeObserver`, so the desktop fits the stage without a phantom horizontal scrollbar); right controls; size presets + deferred free W×H/X/Y px with range validation; preview drag/resize paints via DOM and commits on pointer-up; arrow nudge (10 px); card title uses application name; hidden for `external_browser`.
  - **Multi-app geometry overlays** (session-only): operator may overlay other profile `electron_window` apps on the adaptive-scale preview (peer W×H/x/y as read-only cards; Layers icon top-right on the desktop opens a checkbox multiselect menu with check left of app name); never persisted to settings / F-030; current app and browser-mode peers excluded.
...
- Implementation evidence (adaptive geometry preview scale 2026-08-03): preview divisor adapts to stage content width (min 4 / max 8) for desktop-only fit (no sidebar reserve); same scale drives desktop, primary card, peer overlays, and drag/resize math; stage centers the desktop horizontally.
- Implementation evidence (geometry preview UX polish 2026-08-03): stage `justify-content: center`; UI Kit `DropdownMenuCheckboxItem` always reserves leading indicator gutter so overlay peer names do not overlap the check.
...
- Implementation evidence (OCP socket → NC only 2026-08-02): `mapOcpNotificationToToastDescriptor` uses wire `body`+`type` only (`success`/`error`; else `info`); ignores `deleted`/`blocked`/`sticky`/`position`/`time`/`uuid` for presentation; empty body skipped; tags `ocp`/`ocp.notification`/`remote`; SoftphoneReadyShell → `notify` only; tests `createOcpToastNotificationPresenter.test.ts`; policy docs `notification-center/03-POLICY-AND-CHANNELS.md`
- Implementation evidence (audit remediation 2026-08-02): journal IO failure keeps Domain policy (`journalPersisted`); Capture structured logs; Appearance `closable` → Sonner; catalog `externalApplications`; journal `suppressReasons`; telephonyFocus preset; renderer-wide `notificationProducerTagging.test.ts`; docs `notification-center/13-AUDIT-REMEDIATION.md` + ADR-0025 amend; WU-09 OS still deferred; no SemVer
