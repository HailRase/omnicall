...
- DI-05 follow-up (2026-07-26): OCP ACD `queueLabel` additive on public `call:*` + snapshot call summaries (from `CallOcpContextResolved` / `CallOcpContextProjection`); no `acallid`; no campaign events; docs `OCP-Call-Context.md`, ADR-0017 O-OCP-1, `axatalk-sdk/docs/guide/events.md`
...
- Implementation evidence (E-10 / F-011 SDK queueLabel 2026-07-26): Domain `CallOcpContextResolved`; bridge publishes on non-empty queue; `ExternalSdkEventMapper` / snapshot assembler additive `queueLabel`; protocol optional field on call events + `SnapshotCallSummary`; no DOM CustomEvent; campaign still ADR-0017 deferred; tests mapper/assembler/bridge
