...
- Acceptance Criteria:
  ...
  - **Shell window layout (F-016):** compact mode anchors bottom-right on startup; settings overlay expands window to 1000px width centered; closing restores prior compact width and height at bottom-right; **close motion is owned by BrowserWindow bounds** (280ms `settings-close` easing) — settings overlay stays **fully opaque** for the same duration (no opacity/slide exit fade) so underlay chrome does not flash; `prefers-reduced-motion` skips window animation and unmounts overlay immediately; compact mode disables user resize; settings mode enables user resize; **settings-only maximize** fills the display work area; user restore (unmaximize) returns to settings minimum size (1000×session height, centered); **closing while maximized** animates directly from work-area bounds to compact **without** an intermediate settings-min snap; maximize is unavailable in compact and video-fullscreen modes; shell titlebar suppression and OCP sign-in density stay on settings chrome until overlay hold ends (`onVisibleChange`).
  ...
  - **Shell lifecycle controls (F-016, LF-079):** no native File/View/Window/Help menu on Windows/Linux; macOS keeps minimal App + Edit menus (Edit roles wire Cmd+C/V/A/X/Z in inputs); macOS dev builds add a minimal View menu with `toggleDevTools` (Cmd+Option+I); Windows/Linux dev builds wire F12 and Ctrl+Shift+I via `before-input-event`; `webPreferences.devTools` is enabled only when `!app.isPackaged`; `fullscreenable` disabled; `maximizable` enabled only in settings layout mode; Windows/Linux use native-like titlebar controls `Minimize -> Maximize/Restore (settings only) -> Reload -> Close`; macOS uses custom traffic-light controls `Close -> Minimize -> Reload` in compact mode and `Close -> Minimize -> Maximize` (green) plus reload icon in settings; reload on macOS compact has no tooltip and replaces the green traffic-light slot; close/quit/reload run `ShutdownCleanupUseCase` before `app.quit()` or `app.relaunch()`; cleanup failure blocks quit/restart, cancels pending main shutdown state, and surfaces `shell.shutdown.failed`; facade-not-ready shutdown is acknowledged with cleanup skipped to prevent hangs; force quit/kill cannot guarantee async SIP/legacy operator logout.
- Test Coverage:
...
- Implementation evidence (settings-only maximize **2026-07-30**): `resolveShellWindowMaximizable` / `resolveShellWindowMinimumSize`; `ShellWindowController.toggleMaximize` restores to settings min bounds (1000×session height); IPC `shell:window-toggle-maximize` / `shell:window-maximized-changed`; `ShellWindowControls` maximize on Win/Linux/macOS only when settings open; icons `shell.window.maximize` / `shell.window.restore`
- Implementation evidence (settings close anti-flicker **2026-07-30**): `ShellWindowController.exitMaximizedForLayoutTransition` animates maximized→compact from work-area bounds without settings-min snap; `SettingsFullscreenOverlay` opaque hold via `SETTINGS_SHELL_LAYOUT_ANIMATION_MS`; `onVisibleChange` defers shell chrome/density until unmount; CSS `.panel-holding` / `.backdrop-holding` (no exit opacity); contract in `UI-Architecture.md` (Settings close motion)
...
- Acceptance Criteria:
  ...
  - Settings → Integrations is a parent nav group (**OCP Module** + **External Services** children; F-031). **OmniCall Kit** is a top-level nav leaf immediately below Integrations. Pre-auth: **OCP Module** and **External Services** gated, **OmniCall Kit reachable** (ADR-AF-005 / ADR-AF-004 amended by ADR-0018).
  ...
  - **OCP sign-in progress overlay (global):** any OCP-backed sign-in attempt — Account Login, modal Reconnect, or SDK `account:activate-profile` (OCP mode) — opens the same blurred Dialog with per-stage timed Progress bars (timeout-synced fill, green complete / red failed), per-status Lucide icons (animated when available; active uses centered CSS spin), generic failed status + failure IconTooltip detail, footer **Disconnect OCP** (outline while in progress / destructive on failure; `cancelOcpSignInAttempt` → pre-login idle) and **Reconnect** (enabled on revealed stage failure). Mounted once in `SoftphoneReadyShell` / `AuthAccountShell` (not inside Settings `AccountPanel`); visible on dialpad, contacts, history, and settings. Density: `comfortable` while settings overlay is open (~440px), `compact` on the main compact window (smaller type/gaps/icons, Dialog `sm`). Open gate: `shouldOpenOcpSignInProgressModal` from live `authorizationProgress` (plus optimistic open on manual OCP Login).
...
- Context: Settings
- Priority: high
- Status: **implemented** (2026-07-24; F-031 External Services nested slice extended 2026-07-29)
- Owner: TBD
- Related: **F-031** (nested `UserSettings.externalServices` round-trips inside the same v1 bundle; journal excluded)
...
- Acceptance Criteria:
  ...
  - Outer bundle stays `PREFERENCES_EXPORT_FORMAT_VERSION = 1`; nested schema follows `SETTINGS_SCHEMA_VERSION` (v12+).
...
- Priority: high
- Status: **in-progress** (WU-11 real event integration complete 2026-07-30; closeout → WU-12)
- Owner: TBD
...
- Acceptance Criteria:
  ...
  - Profile switch/logout cancels pending old-profile jobs; in-flight may finish to the captured journal; F-030 round-trips config (journal excluded).
  - Empty default is inert; SIP-only bootstrap unchanged; no F-011/F-028/SIP/headset/transfer behavior mutation.
- Test Coverage:
  - Unit: WU-01 settings parse/migrate; WU-02 HTTP/journal/file mock contracts; WU-03 matcher/template/request/security + event mapper; WU-04 queue FIFO/max-3, execute classification/redaction/journal isolation, manual Run now, IPC parsers, main redirect/timeout; WU-05 file journal parse/cap/corrupt, Save/Query Use Cases; WU-06 PreferencesExportDocument + OperatorPreferences Use Cases External Services round-trip; WU-07 ExternalServiceCollectionDocument + Export/Import Use Cases + collection-file IPC parsers; WU-08 collection mutation helpers + collections panel projection; WU-09 request mutation helpers; WU-10 journal panel projection cap/redaction/truncation
  ...
  - Component / Storybook: WU-08 Postman-like sidebar tree + workspace (`ExternalServicesSidebar`, `ExternalServicesPanel`, Settings Integrations leaf, light/dark stories); WU-09 request URL bar/tabs/Send/Response (`ExternalServicesRequestsEditor.test.tsx`, light/dark stories); WU-10 journal History tab (`ExternalServicesJournal.test.tsx`, light/dark stories); UI polish 2026-07-30 layout skeleton + 2026-07-30 density/rename/body-radio/response-pane polish without feature downgrade
  - E2E: deferred to manual acceptance in `external-services-plan/11-ACCEPTANCE.md` / WU-12
...
- ADR: `docs/softphone/adr/ADR-0022-external-services-http-isolation.md` (**Accepted**; HTTP limits/redirects implemented in WU-04)
