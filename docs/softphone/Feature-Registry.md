...
- DI-03 evidence: `axatalk-sdk-integration/evidence/DI-03-loopback-websocket-transport.md`
- DI-04 evidence: `axatalk-sdk-integration/evidence/DI-04-pairing-origin-capabilities.md`
- DI-05 evidence: `axatalk-sdk-integration/evidence/DI-05-read-only-snapshot-events-window-show.md`
- DI-05 follow-up (2026-07-23): operator public events + coarse-advance — `axatalk-sdk-integration/evidence/DI-05-operator-events-coarse-revision.md`; mapper `ExternalSdkEventMapper` + `SdkOperatorEventRevisionGate`; bind via `bindSdkBrokerSession`
- DI-05 follow-up (2026-07-26): OCP ACD `queueLabel` additive on public `call:*` + snapshot call summaries (from `CallOcpContextResolved` / `CallOcpContextProjection`); no `acallid` on `call:*`; docs `OCP-Call-Context.md`, ADR-0017 O-OCP-1, `axatalk-sdk/docs/guide/events.md`
...
- DI-05/DI-07 follow-up (2026-07-23): post-call reservation observability — optional `reservedTarget`/`reservedReasonId` on snapshot + `operator:status-changed`; `OperatorStatusChangeKind`; no separate reserve command — `axatalk-sdk-integration/evidence/DI-05-operator-reserved-observability.md`; guide `axatalk-sdk/docs/guide/operator-status-reservation.md`
...
- Operator modal TTLs Settings (2026-07-23): Settings → Axatalk SDK → Main selects for consent / Origin TOFU / pairing (`operatorModalTimeouts`); defaults unchanged (120s / 5m / 5m); live gateway + consent bridge via `applyPolicy`; activate hop ceiling `SDK_ACTIVATE_CLIENT_TIMEOUT_MS` = **420s** (max consent); ADR-0018 §G.7
...
- Acceptance Criteria:
  ...
  - Settings → Integrations is a parent nav group (OCP Module child). **Axatalk SDK** is a top-level nav leaf immediately below Integrations. Pre-auth: **OCP Module gated**, **Axatalk SDK reachable** (ADR-AF-005 / ADR-AF-004 amended by ADR-0018).
...
- Implementation evidence (ADR-0018 Axatalk SDK nav, 2026-07-21): top-level `integrations-sdk` leaf below Integrations group; pre-auth SDK enabled, Integrations/OCP gated (`settingsSections.ts`, `SettingsSidebar.test.tsx`)
