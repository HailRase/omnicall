...
- Acceptance Criteria:
  ...
  - **UI (2026-06-30):** `IncomingCallSessionCard` in call context zone — selectable green session card with «Ответить»/«Отклонить»; auto-select on ring; ControlsBar hangup rejects when incoming selected.
...
- Acceptance Criteria:
  ...
  - **Settings nav lock:** when SIP is not registered, all settings sections except **Account** are disabled in the sidebar with `settings.nav.disabled.authorizeFirst`; direct routes to locked sections redirect to account.
