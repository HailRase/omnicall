...
- Acceptance Criteria:
  ...
  - **OCP sign-in progress modal:** Account Login in OCP mode opens a blurred Dialog with overall + per-stage timed Progress bars (timeout-synced fill, green complete / red failed), failure IconTooltip reasons, footer Disconnect (idle reset via `cancelOcpSignInAttempt`) and Reconnect (enabled on stage failure; prefers dual-FSM recovery action).
