...
- Acceptance Criteria:
  ...
  - **Toast chrome clearance (LF-060 / F-016, 2026-07-26):** product Sonner toaster must clear frameless window controls on Win/Linux/macOS. `resolveNotificationToasterOffset` feeds the same chrome-safe object to Sonner `offset` **and** `mobileOffset` (compact shell is 360–420px, so Sonner’s `max-width: 600px` mobile path always applies). Top placements use `--incoming-call-banner-top` + trailing/leading `--shell-window-controls-safe-inline-*`; `NotificationViewport.module.css` keeps corner geometry under the mobile media query (no full-bleed into titlebar controls). Same vertical token as incoming-call / update / OCP banners.
  ...
  - **Bootstrap splash (F-016 / LF-002; single-stage 2026-07-25):** only `#boot-splash` for the whole loading/settle/exit path — React drives it via `useBootSplashController` + `bootSplashDom` (no second React loading splash). Brand mid `#42AAFF` (`#6BC4FF`→`#42AAFF`→`#2A8FD9`); Lucide Phone mark; brand ball with subtle static volume shading + oval radial shadow (no animated `filter: blur`, no bounce `perspective`/`rotateX`); CSS bounce **1000ms** `linear` + ballistic/parabolic Y samples (`BOOTSTRAP_SPLASH_BOUNCE_MS`); settle via `settleSplashBallMotion` (freeze current pose → ease to rest, no mid-air keyframe teleport); determinate progress (~160ms ticks) → settle → opacity crossfade exit (`beginBootSplashExit`) over ready shell; error uses `BootstrapSplashShell`; contract `docs/softphone/Bootstrap-Splash-Contract.md`; `prefers-reduced-motion`; no SIP/facade / no JS spring libs on production splash; bootstrap gate still awaits `initialize`.
...
- Acceptance Criteria:
  ...
  - Settings → Integrations is a parent nav group (OCP Module child). **OmniCall Kit** is a top-level nav leaf immediately below Integrations. Pre-auth: **OCP Module gated**, **OmniCall Kit reachable** (ADR-AF-005 / ADR-AF-004 amended by ADR-0018).
