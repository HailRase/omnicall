...
- Acceptance Criteria:
  ...
  - **Bootstrap splash (F-016 / LF-002, 2026-07-24):** presentational `BootstrapSplashShell` during `useAccountBootstrap` `loading`/`error` (brand + indeterminate `Progress`, semantic tokens, `prefers-reduced-motion`); pre-React `#boot-splash` in `index.html` + `BrowserWindow.backgroundColor` via `startupSplashColors` (synced with `--color-bg-app`); provisional `data-theme` from `prefers-color-scheme` until settings apply; no SIP/facade logic in splash; bootstrap gate unchanged (still awaits `initialize`).
...
- Implementation evidence (bootstrap splash **2026-07-24**): `BootstrapSplashShell`, `startupSplashColors`, `index.html` `#boot-splash`, `BrowserWindow.backgroundColor`, `App.tsx` loading/error swap; tests `BootstrapSplashShell.test.tsx`; Storybook light/dark; i18n `bootstrap.brand` / `bootstrap.loading`
