...
- Test Coverage:
  ...
  - Component / Storybook: WU-08 navigation + collections UI (`ExternalServicesCollectionsView`, Settings Integrations leaf, light/dark stories); WU-09 request list/editor/result states (`ExternalServicesRequestsEditor.test.tsx`, light/dark stories); WU-10 journal accordion (`ExternalServicesJournal.test.tsx`, light/dark stories)
...
- Implementation evidence (WU-08): Settings leaf `integrations-external-services` beside OCP (SDK top-level preserved); icon `settings.integrations.external-services`; Application collection summaries + mutation helpers; hooks `useExternalServicesShell` / `useExternalServicesActions` / `useExternalServicesPanel`; presentational collections/variables UI; i18n `settings.integrations.externalServices.*` ru/en/fr/de/bg; navigation/panel/component tests + light/dark stories; cross-ref F-016
- Implementation evidence (WU-09): request summaries with method/enabled badges and fast toggle; request editor query/header/body/trigger fields plus save/delete/discard confirmation; `useExternalServicesRequestActions` persists drafts before facade Run now and exposes queued/running/classified feedback; `ExternalServicesRequestsEditor.test.tsx`; `ExternalServicesRequestsEditor.stories.tsx`; locale updates ru/en/fr/de/bg.
- Implementation evidence (WU-10): `deriveExternalServicesJournalPanel`; `useExternalServicesJournal`; `ExternalServicesJournal` / `ExternalServicesJournalEntry` / `ExternalServicesJournalEntryDetail` accordion at collections bottom (newest 100, already-redacted headers as `***`, truncation marker, empty/loading/error/retry); i18n `settings.integrations.externalServices.journal.*` + `trigger.manual_run` ru/en/fr/de/bg; `ExternalServicesJournal.test.tsx` + light/dark stories.
