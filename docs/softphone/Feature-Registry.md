...
- Acceptance Criteria:
  ...
  - **Unified action notifications (LF-060):** renderer uses `NotificationViewport` (UI Kit `ToastProvider`/`ToastRoot`) + `useNotifications`; action outcomes are bridged via `useActionNotifications`; persisted preferences (`notificationPlacement`, `notificationStacking`, `notificationDurationMs`, `notificationClosable`, `notificationMaxVisible`) are stored in `UserSettings` and edited in `SettingsGeneralPanel`.
  ...
  - **Shell lifecycle controls (F-016, LF-079):** no native File/Edit/View/Window/Help menu on Windows/Linux; minimal macOS app menu; `maximizable`/`fullscreenable` disabled; Windows/Linux use native-like titlebar controls `Minimize -> Reload -> Close`; macOS keeps native traffic lights with a dedicated reload titlebar action; close/quit/reload run `ShutdownCleanupUseCase` before `app.quit()` or `app.relaunch()`; cleanup failure blocks quit/restart, cancels pending main shutdown state, and surfaces `shell.shutdown.failed`; facade-not-ready shutdown is acknowledged with cleanup skipped to prevent hangs; force quit/kill cannot guarantee async SIP/OCP logout.
- Test Coverage:
...
- Implementation evidence (LF-060 notifications **done**): `NotificationViewport`, `NotificationToast` (UI Kit toast primitives), `notificationLevelToToastTone`, `useNotifications`, `useActionNotifications`, `NotificationViewport.test.tsx`, `NotificationSettings`, `validateUserSettings`/`migrateUserSettings` notification fields, `SettingsGeneralPanel` notification controls, `SoftphoneReadyShell` unified action feedback integration (OCP/update/account/call/settings/session flows).
