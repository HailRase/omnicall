...
- Shell attention (2026-07-22): shared `bringBrowserWindowToFront` + `ShellWindowAttentionController`; raise on incoming/outgoing call (IPC), SDK Origin trust / pairing (main), activate consent (`attentionId` per episode → IPC raise); **root `SdkConnectCeremonyModal`** (TOFU → pairing stepper; pairing-only when Origin already allowed); **no** Settings auto-open for pairing; ADR-0013 / ADR-0018 §G / UX blueprint / TEST-MATRIX / SMOKE
...
- Pending lifecycle (2026-07-22): cancel TOFU / deny pairing on disconnect; Origin leave-allowed → deny pending + close WS (no auto-revoke paired); TTL sweeper; waiting Cancel; Approve requires Origin still allowed — `sdkGatewayPendingLifecycle.test.ts`
...
- Operator modal deadline UI (2026-07-23): muted `MM:SS` header countdown on Origin TOFU / waiting / pairing / activate consent; SSoT `sdkOperatorModalTimeouts.ts`; Origin expiry uses `cancelOriginTrust` (not Deny); ADR-0018 §G.6
- Operator modal TTLs Settings (2026-07-23): Settings → OmniCall Kit → Main selects for consent / Origin TOFU / pairing (`operatorModalTimeouts`); defaults unchanged (120s / 5m / 5m); live gateway + consent bridge via `applyPolicy`; activate hop ceiling `SDK_ACTIVATE_CLIENT_TIMEOUT_MS` = **420s** (max consent); ADR-0018 §G.7
