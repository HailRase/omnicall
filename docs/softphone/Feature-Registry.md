...
- Acceptance Criteria:
  ...
  - **Bootstrap splash (F-016 / LF-002; single-stage 2026-07-25):** only `#boot-splash` for the whole loading/settle/exit path — React drives it via `useBootSplashController` + `bootSplashDom` (no second React loading splash). Brand mid `#42AAFF` (`#6BC4FF`→`#42AAFF`→`#2A8FD9`); Lucide Phone mark; oval ground shadow; determinate progress → settle → opacity crossfade exit (`beginBootSplashExit`) over ready shell; error uses `BootstrapSplashShell`; contract `docs/softphone/Bootstrap-Splash-Contract.md`; `prefers-reduced-motion`; no SIP/facade in splash; bootstrap gate still awaits `initialize`.
