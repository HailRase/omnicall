...
- Acceptance Criteria:
  ...
  - **Bootstrap splash (F-016 / LF-002; single-stage 2026-07-25):** only `#boot-splash` for the whole loading/settle/exit path — React drives it via `useBootSplashController` + `bootSplashDom` (no second React loading splash). Brand mid `#42AAFF` (`#6BC4FF`→`#42AAFF`→`#2A8FD9`); Lucide Phone mark; flat brand ball + oval radial shadow (no animated `filter: blur`); CSS bounce **1000ms** `linear` + ballistic/parabolic Y samples (`BOOTSTRAP_SPLASH_BOUNCE_MS`); settle via `settleSplashBallMotion` (freeze current pose → ease to rest, no mid-air keyframe teleport); determinate progress (~160ms ticks) → settle → opacity crossfade exit (`beginBootSplashExit`) over ready shell; error uses `BootstrapSplashShell`; contract `docs/softphone/Bootstrap-Splash-Contract.md`; `prefers-reduced-motion`; no SIP/facade / no JS spring libs on production splash; bootstrap gate still awaits `initialize`.
