...
- Acceptance Criteria:
  ...
  - **Disconnect OCP semantics:** cancels active attempt + credentials apply epoch, disarms transport recovery, disconnects OCP socket, resets Server/Auth/operator/campaign/progress to cold idle. **Keeps** local account session; **does not** call `EndUserSessionUseCase`; **does not** unregister an already established SIP session. Full pre-login idle remains avatar Logout only.
