...
- Acceptance Criteria:
  ...
  - During Post-call processing, Ready/Break selection reserves (same as busy); dropdown footer shows finish-appeal (`Завершить обращение: {reserved|Доступен}`) via `FinishPostCallAppealUseCase` (`intent: apply` to reserved or Ready). Local reserved projection clears when operator returns to idle.
