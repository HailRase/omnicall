...
- Acceptance Criteria:
  ...
  - **OCP sign-in progress modal:** Account Login in OCP mode opens a blurred Dialog (~420px) with per-stage timed Progress bars of equal width (timeout-synced fill, green complete / red failed), per-status Lucide icons (animated when available; active uses centered CSS spin), generic failed status + failure IconTooltip detail, footer **Disconnect OCP** (outline while in progress / destructive on failure; `cancelOcpSignInAttempt` → pre-login idle) and **Reconnect** (enabled on revealed stage failure).
...
- Test Coverage:
  ...
  - Renderer: **WU-04 done** — `AccountPanel` / `SettingsAccountPanel` mode tabs + OCP InputGroup; no Account logout/switch/generic retry; **WU-05 done** — Settings pre-auth gate + edit-only OCP Module; **T-034 done** — System State SIP/OCP tabs own Server/Authorization; Account keeps in-progress recovery actions only; **OCP progress modal** — `OcpSignInProgress` Dialog + live store progress + Disconnect OCP / Reconnect; modal Reconnect → `recoverOcpSignInFromModal` (not `signInAccount`); a11y live status on semantic stage changes
