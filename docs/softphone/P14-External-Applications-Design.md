...
- Layout: desktop preview is horizontally centered in the stage; scale/fit math unchanged.
...
- Trigger: animated Layers icon absolute top-right on the desktop; DropdownMenuCheckboxItem multiselect (check indicator left of app name; UI Kit gutter always reserved); disabled when no eligible peers.
