...
| LF-009 | P08 | UI | Medium | Re-registration timer UI | `RegisterTimer` | P11: projection `showAvatarRecoveryRing` suppresses fullscreen overlay; interim `control-reregister-sip` in header + `RegistrationStatusDot`. Avatar ring UI **deferred** (redesign). |
