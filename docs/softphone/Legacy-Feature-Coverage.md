...
| LF-008 | P08 | Telephony | High | Repeat SIP registration after failure | `DisplayProvider`, user config | F-014: `ConnectionRecoveryOrchestrationService`, `SipRegistrationRetry*`, `ReregisterSipUseCase`, `SipRecoveryOrchestration.integration.test.ts` — `handoffs/P08-SIP-Registration-Retry-Handoff.md`. |
...
| LF-057 | P08 | UI | High | Lost WS overlay | `WSConnectionOverlay` | **Superseded** (ADR-0004, 2026-07-02): `ConnectionOverlay` removed; recovery UX moves to header SIP status line + Settings «Состояние системы». Prior evidence: `handoffs/archive/P08/P08-WU3-Recovery-Overlay-Handoff.md`. |
