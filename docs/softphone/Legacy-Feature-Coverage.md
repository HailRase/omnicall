...
| LF-038 | P07 | Operator | High | Campaign data on incoming call | `useCampaignEvent`, `IncomingCallOverlay` | **ADR-0005** legacy removed. Parity via **F-028**: `progressiveContext` badges on Incoming/Call session surfaces. |
| LF-039 | P07 | UI | High | Non-progressive campaign request modal | `CampaignEventModal` | **ADR-0005** legacy removed. Parity via **F-028**: `OcpCampaignEventModal` (preview only, center + blur); shell raise on offer via `useShellWindowAttentionFromCampaign` / ADR-0013 (2026-07-26). |
