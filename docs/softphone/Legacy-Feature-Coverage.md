...
| LF-051 | P12 | Integration | High | External call button block | `setCallButtonDisabled` | Capability/command policy landed DI-06; DI-10 packaged handshake/hostile evidence partial — **not closed** until full smoke; see `omnicall-kit-integration/evidence/DI-10-compatibility-e2e-p12-close.md`. No browser global. |
...
| LF-065 | P12 | Integration | Critical | External call events to legacy platform (removed) | `externalEvents`, `legacy events hook` | Per-client redacted public DTO events landed DI-05; OCP queue enrichment via additive `queueLabel` on `call:*` (2026-07-26, replaces legacy `OCP*` CustomEvents — not closed until DI-10 smoke); evidence `DI-10-compatibility-e2e-p12-close.md`, `OCP-Call-Context.md`. |
...
| LF-080 | P12 | Integration | Critical | `window.Softphone` external API | multiple files | **Not ported** (intentional). Replacement gateway DI-01…DI-09 `done`; DI-10 packaged transport/security subset PASS, product pair/call smoke OPEN — LF **not closed** until F-011 `implemented`. Evidence: `DI-10-compatibility-e2e-p12-close.md`. |
| LF-081 | P12 | Integration | High | `legacy status module` external status API | `uselegacy status selectorAPIAdapter` | Public operator DTO/command landed DI-07 (`callType: 'sdk'` at Facade; OCP wire `function_call_type: 'external'` via `mapOcpCallTypeToWire`, 2026-07-23); DI-10 live OCP/operator packaged smoke **OPEN** — LF **not closed**. |
