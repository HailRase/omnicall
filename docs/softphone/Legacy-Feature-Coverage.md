...
| LF-002 | P01 | UI | Critical | Legacy loading screen → SIP-only bootstrap splash | `SoftPhonePlug` → `BootstrapSplashShell` | Startup shows deterministic branded loading splash during app bootstrap (`loading`/`error`); no legacy operator auth gate. |
...
| LF-065 | P12 | Integration | Critical | External call events to legacy platform (removed) | `externalEvents`, `legacy events hook` | Per-client redacted public DTO events landed DI-05; DI-10 packaged paired event smoke **OPEN** — **not closed**; evidence `DI-10-compatibility-e2e-p12-close.md`. |
