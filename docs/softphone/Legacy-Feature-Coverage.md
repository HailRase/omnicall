...
| LF-051 | P12 | Integration | High | External call button block | `setCallButtonDisabled` | Planned through versioned SDK capability/command policy; ADRs ADR-0009/0011/0012; execution package `axatalk-sdk-integration/WORK-UNITS.md` (DI-06). No browser global or direct store mutation. |
...
| LF-065 | P12 | Integration | Critical | External call events to legacy platform (removed) | `externalEvents`, `legacy events hook` | Planned as per-client redacted public DTO events, never raw Domain Events; ADR-0012; execution package `axatalk-sdk-integration/WORK-UNITS.md` (DI-05). |
...
| LF-080 | P12 | Integration | Critical | `window.Softphone` external API | multiple files | **Not ported.** Legacy embed global rejected; external browser tabs will use secure `ExternalClientGateway` in main → typed main/renderer broker → `ExternalCommandRouter` (ADR-0009…0013). Plans: `axatalk-sdk/README.md`, `axatalk-sdk-integration/README.md`, `handoffs/P12-External-Host-API-Master-Handoff.md`. OCP command baseline: F-028 E-12 Facade methods. DI-00 architecture gate `done`. |
| LF-081 | P12 | Integration | High | `legacy status module` external status API | `uselegacy status selectorAPIAdapter` | Planned public operator DTO/command via `ExternalCommandRouter` → `ChangeOperatorStatusUseCase` with `callType: 'sdk'`; no OCP wire types cross the protocol (ADR-0012; `axatalk-sdk-integration/WORK-UNITS.md`, DI-07). |
