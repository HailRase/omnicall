...
| LF-012 | P03 | Media | Critical | Incoming call ringtone | `soundManager`, `newRTCSession` | Incoming call starts ringing through Media service. **F-033 (2026-08-01):** selectable catalog (`incomingRingtoneId`, default `classic`); Settings → Sessions select + preview; Tone FSM unchanged. **Classic (2026-08-02):** FM WebAudio ring (660 Hz + LFO cadence); other presets remain original F-033 step profiles. |
...
| LF-037 | P07 | Operator | High | Queue name display | `useQueueInfoListeners` | **ADR-0005** legacy removed. Parity via **F-028** (2026-07-26): `CallOcpContextProjection` + `deriveCallContextBadges` + `CallContextBadges`; external hosts via SDK additive `queueLabel` (F-011, not `OCPincomingCallProgress`); see `OCP-Call-Context.md`. |
| LF-038 | P07 | Operator | High | Campaign data on incoming call | `useCampaignEvent`, `IncomingCallOverlay` | **ADR-0005** legacy removed. Parity via **F-028**: `progressiveContext` badges on Incoming/Call session surfaces. SDK notify: `operator:campaign-offered` (`mode: progressive`) + snapshot `operator.campaign` (ADR-0019 / `operator.campaign.read`). |
| LF-039 | P07 | UI | High | Non-progressive campaign request modal | `CampaignEventModal` | **ADR-0005** legacy removed. Parity via **F-028**: `OcpCampaignEventModal` (preview only, center + blur); shell raise on offer via `useShellWindowAttentionFromCampaign` / ADR-0013 (2026-07-26). SDK notify: `operator:campaign-offered` (`mode: preview`); accept/reject remain desktop UI (no public SDK commands in v1). |
| LF-040 | P07 | Operator | High | Campaign answer or reject update | `CampaignEventModal` -> WS update | **ADR-0005** legacy removed. Parity via **F-028**: `AcceptCampaignUseCase` / `RejectCampaignUseCase`. |
...
| LF-051 | P12 | Integration | High | External call button block | `setCallButtonDisabled` | **Closed** with F-011 `implemented` (2026-07-27). Capability/command policy DI-06; DI-10 full close — `omnicall-kit-integration/evidence/DI-10-compatibility-e2e-p12-close.md`. No browser global. |
...
| LF-058 | P08 | Operator | High | WS reconnect 6 attempts by 5 seconds | `useWs` | WU2: `legacy reconnect policy`, `Legacy reconnect*`, orchestration + integration test — see `handoffs/archive/P08/P08-WU2-Recovery-Orchestration-Handoff.md`. **F-028 / ADR-AF-002 (2026-07-16):** Application `OcpTransportRecoveryService` owns capped fresh-HTTP-token reconnect; `OcpWebSocketAdapter` no longer schedules stale-token reconnect. **2026-07-17:** fresh-token HTTP host is OCP proxy domain only — `entity:creds` SIP domain must not replace it (`resolveOcpProxyAuthenticateDomain`). **2026-07-30:** attempt budget preserved during in-flight recovery connect disarm; `transportRecoveryActive` keeps banner across disconnect/HTTP/WS flaps; UI is global `OcpConnectionBanner` in shell overlays (`uiSurface: silent`, `--z-shell-status-banner` above Settings — no six-stage sign-in Dialog / no duplicate token toasts on auto-recovery). **2026-08-06:** modal progress reveals failures immediately; banner Retry = dual-FSM `retry_server` with login cascade (ADR-AF-002); compact one-line chip + outline Retry geometry for ≥360px main display (`UI-Design-System.md` § OCP connection banner). |
...
| LF-060 | P11 | Settings | Low | Toast position and z-index settings | user config | User config controls notification placement. `NotificationViewport` keeps every placement within the BrowserWindow at compact widths and during compact↔Settings resize; top toasts clear titlebar controls vertically and use a 24px horizontal edge. Evidence: `resolveNotificationToasterOffset.test.ts`, `NotificationViewport.test.tsx`. |
...
| LF-065 | P12 | Integration | Critical | External call events to legacy platform (removed) | `externalEvents`, `legacy events hook` | **Closed** with F-011 `implemented` (2026-07-27). Per-client redacted public DTO events DI-05; OCP `queueLabel` on `call:*`; evidence `DI-10-compatibility-e2e-p12-close.md`, `OCP-Call-Context.md`. |
...
| LF-080 | P12 | Integration | Critical | `window.Softphone` external API | multiple files | **Not ported** (intentional). Replacement gateway DI-01…DI-11 `done`; DI-10 full close; F-011 **`implemented`** 2026-07-27 — LF **closed** via replacement. Evidence: `DI-10-compatibility-e2e-p12-close.md`. |
| LF-081 | P12 | Integration | High | `legacy status module` external status API | `uselegacy status selectorAPIAdapter` | **Closed** with F-011 `implemented` (2026-07-27). Public operator DTO/command DI-07; DI-10 full close. |
