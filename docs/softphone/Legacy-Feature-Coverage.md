...
| LF-009 | P08 | UI | Medium | Re-registration timer UI | `RegisterTimer` | P11: `AvatarRecoveryRing` + `useReconnectCountdown` on header avatar; fullscreen overlay suppressed during SIP registration recovery (`deriveConnectionRecoveryShell.showAvatarRecoveryRing`). |
