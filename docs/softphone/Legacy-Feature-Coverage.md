...
| LF-077 | P11 | Settings | High | Per-user config in local storage | `JSSIP_CONFIGS` | WU4: typed `UserSettings` v3 schema, port aggregate, `FileSettingsRepository` in-memory stub. **F-023 in progress:** disk persistence, profile switch on authorize, composite profile key — `P11-Local-Account-Profiles-Design.md`. |
